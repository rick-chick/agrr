...
| 層 | やること | やらないこと |
|----|----------|--------------|
| **機械**（Actions / `*-dispatch*.mjs`） | GitHub イベント検知 → webhook で Agent 起動。`agent-in-progress` / `agent-merge-in-progress` 等の**重複抑止のみ** | obsolete 判定・close・merge 可否・「触るな」ラベル付与・経路分岐（`conflict` / `ci_fix` / `pr_review` 等） |
| **Agent**（Automation + SKILL） | 毎 run 先頭で `gh` 観測 → close / 修正 / マージ / コメント / exit 0 | 機械が付けた判断印・payload の `action` / `pr_unlinked` 等を**信用して skip** |
...
| webhook payload: `repository`, `issue_number`, `pr_number`（`action` なし） | payload `pr_unlinked` を Agent の skip 根拠にする設計 |

**例外（機械 prep のみ）**: `pr-agent-prep` は `closingIssuesReferences` **あり**の Draft PR を `gh pr ready` する。merge 可否・§0a の判断ではない。

## 4. Agent が信用してはいけないもの

| 入力 | 正しい扱い |
|------|------------|
| `pr_unlinked`（レガシー optional） | `gh pr view` の `closingIssuesReferences` で未リンクか確認 |
| `agent-no-merge` / `do-not-merge` / `wip` | 無視。観測のみで着手可否を決める |
| payload `action` | 送らない。あっても無視 |
| dispatch lib の内部ゲート名（`conflict` / `ci_fix` / `pr_review` / `stuck_retry`） | 実装の残債。Agent の判断根拠にしない |
...
実装除去は別タスク。ドキュメントと Agent の正は §1–§5。

| 残債 | 正しい扱い |
|------|------------|
| `*-dispatch-lib.mjs` の内部ゲート名 | webhook 再送の実装詳細。判断は Agent |
| `agent-no-merge` 付与（reconcile prep 等） | 新規追加禁止。Agent は無視 |
| `pr_unlinked` payload 分岐 | 送ってよいが Agent は信用しない |
| 旧 Automation（Issue Worker / PR Merge Worker 分離） | Delivery Agent に統合済み。新規セットアップ禁止 |

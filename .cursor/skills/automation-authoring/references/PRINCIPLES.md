...
| PR reconcile 前処理 | 未リンク `agent-no-merge`（[`pr-merge-worker-retry-dispatch.mjs`](../../../scripts/pr-merge-worker-retry-dispatch.mjs) `reconcilePrep`）。陳腐化 PR close は **Agent**（内部ゲート `pr_review`） |

...
| 用途 | 正しい経路 |
|------|------------|
| 依存未充足で待つ | `agent-ready` 維持 + dispatch 依存ゲート（**ラベル契約** `agent-deps-ready` / `agent-deps-wait-<N>` のみ。本文・コメントパース禁止。reconcile が再判定） |
...
**本文・コメントパースは禁止。** regex・grep・ヒューリスティックは **Cursor Agent（Worker SKILL）のみ**。機械層は GitHub API 構造フィールドと **ラベル契約**（`agent-deps-ready`、`agent-deps-wait-<N>`、Memory）だけを読む。
...
| 層 | 担当 | 例 |
|----|------|-----|
| **機械ゲート**（Actions / dispatch lib） | 起動・再送・選定の可否だけ | マージ済みのみ、`agent-in-progress` あり → 送らない、`agent-deps-ready` 欠落 → deps resolve |
| **エージェント判定**（Automation + SKILL） | 本文・コメント・スコープ・依存の判断 | `gh` で本文を読む triage、epic クローズ、依存ラベル付与 |

**機械ゲートの入力**は GitHub API 構造フィールドとラベル契約に限る。本文・コメントテキストのパースは含めない。

**禁止（Actions / dispatch lib）**: 本文パース、**コメントパース**、エージェント triage の workflow bash 複製。

**エージェント判定**: SKILL に従い `gh` で読む。結果をラベル（`agent-deps-ready` / `agent-deps-wait-<N>`）・コメント（人間向け）・Memory に残す。対象外は **exit 0**（理由は Memory またはコメントに短く）。
...
- **依存ゲート**は **ラベル契約のみ**（`issue-worker-deps-agent-lib.mjs` の `agent-deps-ready` / `agent-deps-wait-<N>`）。本文・コメントの regex / `#N` パースは禁止（[§機械ゲートとエージェント判定](#機械ゲートとエージェント判定)）

...
オートメーションの**修正・レビュー・提案**は、症状を止める最小差分より **本書の原則に沿った案**を先に示す。

| 優先 | 意味 |
|------|------|
| **1. 思想に沿う完成形** | 二層分離・全部拾う・人間介在なしが説明できる差分 |
| **2. 最小パッチ** | 上を満たしたうえでの最小 |

**禁止**: 「動いている」「差分が小さい」「既存もパースしている」を理由に、機械層の本文パース・狭いスキップ・責任空白を残すこと。[`automation-philosophy-priority.mdc`](../../../rules/automation-philosophy-priority.mdc) を常時適用。

オートメーション経路を**触る変更**では、触れた範囲の**既存本文パースも同一変更で除去**する（後回し・別 PR は規約上の障害時のみ）。
...
| 用途 | 正しい経路 |
|------|------------|
| 依存未充足で待つ | `agent-ready` 維持 + dispatch 依存ゲート（`agent-deps:v1` キャッシュのみ。本文 `#N` パース禁止。reconcile が再判定） |
...
**本文のパースは禁止。** issue / PR 本文を regex・grep・ヒューリスティックで読むのは **Cursor Agent（Worker SKILL）だけ**が行う。必要な判断はエージェントが `gh` で本文を読む。機械層で省略・代用しない。

**総コストの比較では、判断が要る場面ではエージェント起動を優先する。** 本文パースや状態ラベル追加だけで dispatch を減らす設計は、長期的な総コストが大きくなる傾向がある。判断が要るときは webhook で Agent を起動し、SKILL に従わせる。

**パース・例外目的の追加ラベルによるエージェント起動の抑制は設計として用いない。** エージェントで解ける判断を機械層に移すと、改修の繰り返し・滞留・負債解消を含む総コストが、エージェント起動のみの経路より大きくなることが、本リポジトリの改修履歴で繰り返し確認されている。

**エージェントコストの低減**は、上記以外の手段で行う: 構造フィールドのみの機械ゲート、dispatch lib の単一化、reconcile の 1 件選定、SKILL 明確化、watchdog・契約テスト・concurrency／backoff。本文パース・例外ラベル・dispatch 省略で一見コストが下がる設計は禁止。

GitHub Actions / `*-dispatch-lib.mjs` は **狭い機械ゲート**だけを担う（イベント・マージ可否・in-progress・キャッシュ有無・reconcile 選定など）。二層を混ぜない。

| 層 | 担当 | 例 |
|----|------|-----|
| **機械ゲート**（Actions / dispatch lib） | 起動・再送・選定の可否だけ | マージ済みのみ、`agent-in-progress` あり → 送らない、`agent-deps:v1` 欠落 → deps resolve |
| **エージェント判定**（Automation + SKILL） | 本文・スコープ・依存・対象外の判断 | `gh` で本文を読む triage、epic クローズ、UX キャンペーン対象か、依存の `agent-deps:v1` 作成 |

**機械ゲートの入力**は GitHub API の構造フィールドと、**エージェントが先に書いた契約**（ラベル、`agent-deps:v1` コメント、Memory）に限る。本文テキストのパースは含めない。

**禁止（Actions / dispatch lib）**

- issue / PR 本文のパース（`Closes #N` 抽出、キーワード一致、依存 `#N` の regex を含む）
- エージェントがすべき triage を workflow bash に複製する

**エージェント判定**

- 本文の読解が要るときは SKILL に従い `gh pr view` / `gh issue view` で読む（機械層で代用しない）
- 結果は次回の機械ゲートが読める形に残す: ラベル、`agent-deps:v1`、コメント、Memory
- 対象外なら **exit 0**（workflow を失敗にしない）。理由は Memory またはコメントに短く残す
...
機械層に本文パースが**既にあって動いていても技術負債**。オートメーション経路の変更・レビューで発見したら **直ちに解消**する（エージェント判定への委譲、GitHub API 構造フィールドへの置換）。「レガシーだから触らない」は [`no-convenience-tech-debt.mdc`](../../../rules/no-convenience-tech-debt.mdc) 違反。

## ゲート

- **起動判定・選定**は `scripts/*-dispatch-lib.mjs` に集約する（webhook payload に `action` は載せない。Delivery Agent が GitHub を観測して分岐）
- **依存ゲート**は `agent-deps:v1` コメントキャッシュのみ（`issue-worker-deps-agent-lib.mjs`）。`## 依存` 本文の regex / `#N` パースは禁止（[§機械ゲートとエージェント判定](#機械ゲートとエージェント判定)）

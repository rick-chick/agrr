...
**本文のパースは禁止（零容認）。** issue / PR 本文を regex・grep・ヒューリスティックで読むのは **Cursor Agent（Worker SKILL）だけ**が行う。必要な判断はエージェントが `gh` で本文を読む。**エージェント判断は本文を的確に解釈できる** — 機械層で省略・代用しない。

**エージェント起動のコスト**は、本文パースや状態ラベルで dispatch を減らすコストよりはるかに価値がある。判断が要るときは webhook で Agent を起動し、SKILL に従わせる。

**パース・ラベル穴あけによるエージェント起動の抑制は、最初から採用に値しない。** エージェントで解ける判断を機械層に移すと、改修の繰り返し・滞留・負債解消の総コストが支配的になる（本リポジトリで繰り返し観測された失敗パターン）。

**エージェントコストの低減**は、上記以外の手段で行う: 構造フィールドのみの機械ゲート、dispatch lib の単一化、reconcile の 1 件選定、SKILL 明確化、watchdog・契約テスト・concurrency／backoff。本文パース・ラベル穴・dispatch 省略で「安く」見せる設計は禁止。

...
**本文パースは禁止。** regex・grep・ヒューリスティックは **Cursor Agent（Worker SKILL）のみ**。機械層は GitHub API 構造フィールドとエージェント契約（ラベル、`agent-deps:v1`、Memory）だけを読む。
...
**機械ゲートの入力**は GitHub API 構造フィールドとエージェント契約に限る。本文テキストのパースは含めない。

**禁止（Actions / dispatch lib）**: 本文パース、エージェント triage の workflow bash 複製。

**エージェント判定**: SKILL に従い `gh` で読む。結果をラベル・`agent-deps:v1`・コメント・Memory に残す。対象外は **exit 0**（理由は Memory またはコメントに短く）。

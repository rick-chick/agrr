...
BACKDOOR_TOKEN_SERVICE=$(gcloud run services describe $SERVICE_NAME --region $REGION --project "${PROJECT_ID:-}" --format 'value(spec.template.spec.containers[0].env[?(@.name=="AGRR_BACKDOOR_TOKEN")].value)' 2>/dev/null || echo "")

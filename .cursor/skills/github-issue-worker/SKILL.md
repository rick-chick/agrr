...
3. 完了条件を PR 本文に写し、各行に確認根拠を付ける:
   ...
   - 根拠なしで `[x]` にしない
4. issue 本文の**必須**セクションに列挙された完了条件について、`[ ]`・`未カバー`・`手動未実施` が残る → `Closes #N` を付けない（`Part of #N` または follow-up issue）
...
- `Closes #N` は §5 で issue 必須セクションの完了条件がすべて `[x]` のときのみ（テンプレは未達例のため `Part of #N`）

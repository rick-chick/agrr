...
| `agent-ready` ラベル（`[epic]` / `epic`） | `epic_close_check` | **エピック本体**。§1b で子 issue の完了を確認し、全 CLOSED なら §2a で epic をクローズ |
...
**主要 issue の triage が終わったら必ず実施**する（実装着手・§2a クローズ・deps_unmet のいずれの経路でも）。`action: epic_close_check` のときは **§1b が主要タスク**（§3 以降の実装には入らない）。

### A. 対象が `[epic]` / `epic` ラベル（`epic_close_check`）

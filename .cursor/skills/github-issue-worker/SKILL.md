...
### dispatch lib 内部ゲート名（webhook には送らない）

[`issue-worker-dispatch-lib.mjs`](../../../scripts/issue-worker-dispatch-lib.mjs) の機械的ゲート名。reconcile / dispatch 層のみが使用。**Webhook payload のフィールドではない**。

| 内部 `action` | 対応トリガー |
|---------------|--------------|
| `triage` | issue `opened` |
| `implement` | `agent-ready` ラベル |
| `epic_close_check` | `agent-ready` かつ `[epic]` / `epic` ラベル |
| `close_with_reason` | `agent-close` ラベル |

ペイロード例: `repository`, `issue_number`（任意: `issue_title`, `issue_url`, `issue_body`, `labels`）
...
**`agent-close` ラベル**（内部ゲート `close_with_reason`）のときは **§2a のみ**（実装・PR 禁止）。

**`agent-ready` ラベル**（内部ゲート `implement`）のときも **着手前 triage は必須**（既に fixed なら §2a / 依存未充足ならコメントのみ）。

**issue `opened`**（内部ゲート `triage`）のときは上記分岐のいずれかで終了する（cron 的な「対象なしで何もしない」は使わない）。
...
**主要 issue の triage が終わったら必ず実施**する（実装着手・§2a クローズ・deps_unmet のいずれの経路でも）。**エピック本体**（`agent-ready` + `[epic]` / `epic` ラベル、内部ゲート `epic_close_check`）のときは **§1b が主要タスク**（§3 以降の実装には入らない）。

### A. 対象が `[epic]` / `epic` ラベル（内部ゲート `epic_close_check`）
...
- **`agent-close` ラベル**（内部ゲート `close_with_reason`）指定時の実装・PR

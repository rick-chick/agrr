...
**廃止予定**: 専用 Issue Worker Automation は [`delivery-agent/SKILL.md`](../delivery-agent/SKILL.md) に統合。切替後は Delivery の Webhook のみ使用（`action` なし payload）。
...
Webhook payload fields: issue_number, action (triage | implement | epic_close_check | close_with_reason), issue_body, labels.

- action triage: new issue opened — run §1 triage (implement or §2a close only; no human wait). Finish with §1b.
- action implement: agent-ready label — implementation path after triage. Finish with §1b.
- action epic_close_check: agent-ready on epic — §1b only (close epic if all children done; never open a PR).
- action close_with_reason: agent-close label — §2a only. Finish with §1b when parent epics may be closable.
...
|----------|-------------------|
| issue 作成 | `triage` |
| `agent-ready` 付与 | `implement`（epic は `epic_close_check` にリマップ） |

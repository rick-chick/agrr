...
PR_JSON=$(gh pr view "$PR_NUMBER" --json headRefName,mergeable,mergeStateStatus,isCrossRepository,headRepositoryOwner)
...
if [ "$IS_CROSS_REPO" = "true" ] || [ "$HEAD_OWNER" != "$BASE_OWNER" ]; then

...
| Cursor Automation（webhook） | `.github/workflows/pr-merge-worker-dispatch.yml`（**Backend test 完了時** + PR opened / `agent-merge` label + **master push 後のコンフリクト検知**） |
| 手動 | 「PR #N をマージワーカー」「#123 をマージ可能にして」 |

Webhook payload フィールド: `repository`, `pr_number`, `pr_title`, `pr_url`, `action`（`opened` | `labeled` | `ci_completed` | **`conflict`**）, `head_ref`, `head_sha`, `author`, `mergeable_state`, `merge_state_status`（`conflict` 時）。

### `action: conflict`（master 更新後）

`master` へ push されたあと、対象 PR が `BEHIND` / `DIRTY` / `CONFLICTING` のとき `.github/workflows/pr-merge-worker-dispatch.yml` が **CI 完了を待たず** dispatch する（候補選定: `scripts/pr-merge-worker-needs-sync.mjs`）。
...
### 5.1) コンフリクト解消（`action: conflict` または `mergeable` が `CONFLICTING`）

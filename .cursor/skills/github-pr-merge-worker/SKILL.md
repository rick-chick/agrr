...
本 Worker は **ready 済み**の PR を **マージ** する（Draft はマージしない）。**例外**: `action: conflict` / master sync（`BEHIND` / `DIRTY` / `CONFLICTING`）では **Draft でも着手**（コンフリクト解消のみ）。**例外**: `action: ci_fix` では **Draft/ready 問わず** 必須 CI FAIL（コンフリクトなし）でも着手（§5 修正ループのみ。マージはしない）。

### `action: ci_fix`（必須 CI 失敗 — 全 PR）
...
- **Draft**（`mergeable` / `mergeStateStatus` が `CONFLICTING` / `DIRTY` / `BEHIND` のとき、および `action: ci_fix` は **除外しない**）

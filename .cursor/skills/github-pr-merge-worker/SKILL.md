...
本 Worker は **ready 済み**かつオプトイン対象の PR のみ **マージ** する。**例外**: `action: conflict` / master sync（`BEHIND` / `DIRTY` / `CONFLICTING`）では **Draft でも着手**（コンフリクト解消のみ。マージはしない）。**例外**: `action: ci_fix` では **Draft + `agent-merge` + 必須 CI FAIL**（コンフリクトなし）でも着手（§5 修正ループのみ。マージはしない）。

### `action: ci_fix`（Draft + 必須 CI 失敗）

Issue Worker が open `fixes`/`closes` PR を持つため再 dispatch できず、pr-agent-prep も CI green まで ready 化しない **責任空白**（#354）を埋める経路。

| 条件（すべて） | 説明 |
|----------------|------|
| PR が **Draft** | Issue Worker 由来の Draft PR |
| ラベル **`agent-merge`** | pr-agent-prep 付与済み |
...
| **マージ** | **本 run ではマージしない**（CI green 後は pr-agent-prep が ready 化 → 通常 `ci_completed` 経路） |
...
| Webhook `action: ci_fix` | **CI 待ちをスキップ**（dispatch 側が FAIL 済みを確認）。§0 → §5 修正ループへ（マージはしない） |
| Webhook `action: stuck_retry` | **CI 待ちをスキップ**（retry dispatch が green を確認済み）。§0 の `agent-merge-in-progress` は **stale 除去後**の再 dispatch を想定 → §3 へ |

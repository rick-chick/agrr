...
検出した逸脱は修正単位として**省略せず**追跡可能な形に残す（会話での列挙・コミット説明・契約／ADR など、チームが追える手段でよい）。「今回だけ」「後で」「スコープ外」を理由に **`ARCHITECTURE.md` と矛盾する未対応のまま残すこと**をしない（[`no-convenience-tech-debt.mdc`](../../rules/no-convenience-tech-debt.mdc)）。
...
- 指摘を一度にまとめて反映する。スコープ外のリファクタリングはしない。
...
- 「最近触ったところは違反なし」「直近 PR の差分には違反なし」を終了根拠にしない。終了はリポジトリ全体の現行ファイルに対する洗い出し結果で判断する。

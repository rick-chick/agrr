...
| **UX Campaign Loop** | `ux-campaign-loop` | —（PR マージ後 webhook） | — | 不可（残件起票のみ・完了時に自身を無効化） |
...
ループ: issue（`ux-campaign:breadcrumb`）→ Delivery Agent → PR → Delivery Agent（マージ）→ **本 Automation** → 残件 issue（`agent-ready`）→ … → scan 完了 → Automation 無効化

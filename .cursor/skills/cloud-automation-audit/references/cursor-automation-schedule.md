...
| Automation | スキル | トリガー | TZ | PR |
|------------|--------|----------|-----|-----|
| **Issue Worker** | `github-issue-worker` | **Webhook**（issue `opened` + ラベル） | — | ✅ 実装時 |
...
**Automation Audit** は金曜 10:00 JST。直近 1 週間の **Issue Worker（issue 作成 dispatch）** と月曜 UX Audit を監査する。
...
Issue Worker は **Schedule ではなく Webhook のみ**（下記 §Issue Worker）。

### Prefill URL（フォーム事前入力）

Prefill で **Trigger が Invalid** になる場合は UI から手動でトリガーを入れ直す。

- **Issue Worker（Webhook・issue 作成起点）**: [prefill を開く](https://cursor.com/automations/new?prefill=eyJuYW1lIjoiQUdSUiBJc3N1ZSBXb3JrZXIgKGlzc3VlIOS9nOaIkOi1t-eCuSkiLCJkZXNjcmlwdGlvbiI6InJpY2stY2hpY2svYWdyciDjga4gaXNzdWUg5L2c5oiQ44KSIHRyaWFnZSDjgZcgVERE4oaSUFIg44G-)
...
**目的**: issue **作成**（または `agent-ready`）を起点に 1 件 triage → TDD → PR。対応しない場合は `agent-skipped` または close。

詳細: [github-issue-worker/SKILL.md](../../github-issue-worker/SKILL.md) §セットアップ

**Trigger（Cursor UI）**: **Webhook のみ**（Schedule cron は廃止）

**GitHub Actions**: `.github/workflows/issue-worker-dispatch.yml`

| イベント | payload `action` |
|----------|------------------|
| issue `opened` | `triage` |
| ラベル `agent-ready` | `implement` |
| ラベル `agent-close` | `close_with_reason` |
...
未設定のまま issue が作成されると `Issue Worker Dispatch` workflow が **exit 1**（Actions 失敗で気づく）。
...
Webhook payload: issue_number, action (triage | implement | close_with_reason), issue_body, labels.

- triage: new issue — §1 triage then implement / agent-skipped / close / block.
- implement: agent-ready — implementation after triage.
- close_with_reason: agent-close — §2a only.

After TDD GREEN, always run sequential-cleanup-review-workflow (§4) before opening a PR.
One issue per run. Use test-common only. Do not git checkout/switch/reset/restore.
```
...
2. **Webhook** — `.github/workflows/pr-merge-worker-dispatch.yml`（**Backend test 完了時** — ruleset 3 context すべて SUCCESS 後に dispatch + PR `opened` / `agent-merge` ラベル）
...
| Webhook secret 空 | Run test を使用（既知バグ）。Secrets 設定後に issue 作成で E2E 確認 |
| Issue Worker が起動しない | Schedule が残っていないか確認。`Issue Worker Dispatch` Actions と Secrets を確認 |

...
2. **Webhook** — `.github/workflows/pr-merge-worker-dispatch.yml`（**Backend test 完了時** — ruleset 3 context すべて SUCCESS 後に dispatch + PR `opened` / `agent-merge` ラベル + **`master` push 後のコンフリクト / BEHIND 検知**）

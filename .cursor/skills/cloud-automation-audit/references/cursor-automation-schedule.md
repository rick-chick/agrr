...
1. [作成済み Automation](https://cursor.com/automations/6a5cb2d9-8317-11f1-a7d1-d6b4613131ce) を開く（Webhook URL は変えない）
2. プロンプトを [delivery-agent/SKILL.md § Automation](../../delivery-agent/SKILL.md) のブロックに**丸ごと置換**（`pr_unlinked (optional)` を含むこと）
3. **Run test** で `pr_unlinked` 付き payload が受理されることを確認
4. Save（Webhook URL / API key はそのまま — secrets の再登録は不要）

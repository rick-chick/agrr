...
2. **Webhook** — `.github/workflows/pr-merge-worker-dispatch.yml`（**Backend test 完了時** + PR `opened` / `agent-merge` ラベル）
3. **Pull request opened** — 任意（§0 の `agent-merge-in-progress` で重複抑止）
...
**対象 PR（いずれか）**: ラベル `agent-merge` / head `issue/*` / 本文 `Merge-Strategy: agent`

...
| **PR Merge Worker Retry** | GitHub Actions（`pr-merge-worker-retry-dispatch.yml`） | `*/15 * * * *` | — | ❌（再 dispatch のみ） |
...
3. **Retry dispatch** — `.github/workflows/pr-merge-worker-retry-dispatch.yml`（**15 分ごと** + primary dispatch **cancelled** 時。ready + CI green のスタック PR を `action: stuck_retry` で再 dispatch。stale `agent-merge-in-progress` は除去）

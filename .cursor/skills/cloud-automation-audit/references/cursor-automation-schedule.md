...
|------------|--------|----------------------|-----|-----|
| **Issue Worker** | `github-issue-worker` | —（webhook + `issue-worker-dispatch.yml`） | — | 可（実装時） |
...
| **PR Merge Worker** | `github-pr-merge-worker` | —（イベント駆動） | — | 不可（マージのみ） |
| **PR Merge Worker Retry** | GitHub Actions（`pr-merge-worker-retry-dispatch.yml`） | `*/15 * * * *` | — | 不可（統合 reconcile: `conflict` + `ci_fix` + `stuck_retry` 再 dispatch） |
...
**目的**: `master` 向け PR を CI 通過後にレビューし、問題なければ squash マージ。滞留（BEHIND / CI 赤 / stuck）は自動救済。人間のラベル付与を前提にしない（[automation-authoring PRINCIPLES](../../automation-authoring/references/PRINCIPLES.md)）。

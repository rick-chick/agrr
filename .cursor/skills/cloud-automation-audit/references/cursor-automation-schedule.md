...
**切替**: Delivery 作成 → secrets → 旧 Automation OFF → workflow マージ → スモーク。ロールバックは旧 Automation ON + secrets 復元。

## Issue Worker（実装）— レガシー

**廃止予定**: [`delivery-agent`](../../delivery-agent/SKILL.md) に統合済み。
...
| `CURSOR_ISSUE_WORKER_DEPS_WEBHOOK_KEY` | （任意）上記 API key。未設定時はキャッシュ miss で dispatch / retriage がブロック |

トリガー: issue `opened`（`action: triage`）/ ラベル `agent-ready`（`implement`）/ `agent-close`（`close_with_reason`）。retry は `issue-worker-retry-dispatch.yml`（15 分 cron + primary dispatch cancelled/failure）。
...
2. **Webhook** — `.github/workflows/pr-merge-worker-dispatch.yml`（**Backend test 完了時** — green ならマージ経路、必須 CI FAIL なら `ci_fix` + PR `opened` / `ready_for_review` / `agent-merge` ラベル / PR `synchronize` でコンフリクト検知 + **`master` push 後のコンフリクト / BEHIND 検知**。対象は全 PR・オプトアウトのみ）
3. **Retry dispatch** — `.github/workflows/pr-merge-worker-retry-dispatch.yml`（**15 分ごと** + primary dispatch **cancelled/failure** 時。**統合 reconcile** — `conflict` / `ci_fix` / `stuck_retry`。open + base master 全件。`classifyReconcileCandidate` / `selectReconcileCandidate` で 1 件）
...
**対象 PR（既定: すべて / オプトアウトのみ）**: `master` 向け同一リポジトリ PR は既定で対象。除外: `agent-no-merge` / `do-not-merge` / `wip` / `agent-merge-blocked`、fork、`CHANGES_REQUESTED`、タイトル `[WIP]`/`[DRAFT]`。`agent-merge` ラベルは互換のため残すが必須ではない（[github-pr-merge-worker/SKILL.md](../../github-pr-merge-worker/SKILL.md) §1）。

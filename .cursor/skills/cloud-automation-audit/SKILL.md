...
| 名前 | スキル | cron | 期待する成果 |
|------|--------|------|--------------|
| **Delivery Agent** | `delivery-agent` | webhook（dispatch workflows） | issue 実装 PR → prep → squash merge / close / 依存待ち |
| ~~Issue Worker~~ | `github-issue-worker` | — | **廃止**（Delivery に統合。参照スキルのみ） |
| ~~PR Merge Worker~~ | `github-pr-merge-worker` | — | **廃止**（Delivery に統合。参照スキルのみ） |
...
各 Automation（Delivery Agent / UX Audit）は**自分の Memory** に失敗・スキップ理由を書く運用を推奨。監査側はそれを読めないため、§1B で間接確認する。

### B. GitHub 副作用（第一情報源・毎回）

```bash
# Delivery Agent: 直近 7 日の automation 由来 PR（author は cursor または作成者アカウント）
...
# UX Audit 前提（証拠鎖: bundle + 最新 visual-review。コミット済み md の存在は不要）
test -f frontend/e2e/agent-review/agent-review-bundle.json || echo 'skip UX collect: no capture bundle'
...
if test -f frontend/e2e/agent-review/agent-review-bundle.json \
   && test -f frontend/e2e/agent-review/visual-review-results.md; then
...
| **P1 クリティカル** | 主要成果物が毀損（`gh issue list` / `githubLookupStatus: failed` が **2 週連続**、Delivery Agent が再現可能バグで毎回失敗） | 根因が repo 側なら **PR**。Dashboard / Secret のみなら **レポートにエスカレーション** |
...
- Delivery Agent / UX Audit の**業務ロジック**変更（issue 選定・起票閾値等）
...
- Delivery Agent: [`delivery-agent`](../delivery-agent/SKILL.md)
- Issue 実装（参照）: [`github-issue-worker`](../github-issue-worker/SKILL.md)
- PR マージ（参照）: [`github-pr-merge-worker`](../github-pr-merge-worker/SKILL.md)
...
| cron | 意味 |
|------|------|
| `0 10 * * 5` | 毎週金曜 10:00 JST（Delivery Agent webhook run + 月曜 UX Audit の後） |
...
Audit the last 7 days of Delivery Agent and UX Issue Audit runs using GitHub side effects and repository smoke tests (you cannot read other automations' run logs or memories). Fix ONLY critical breakages in the repository. Do NOT make proactive improvements.

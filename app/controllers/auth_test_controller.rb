...
class AuthTestController < ApplicationController
  ...
  def mock_login_as_user(auth_key)
    ...
    if return_to.present? && allowed_return_to?(return_to)
      redirect_to CompositionRoot.oauth_conversion_url_appender.append(return_to), allow_other_host: true, notice: I18n.t("auth_test.mock_login_success", name: user.name)
    else

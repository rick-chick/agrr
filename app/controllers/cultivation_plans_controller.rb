...
class CultivationPlansController < ApplicationController
  ...
  def select_farm_size
    ...
    session[:cultivation_plan] = { farm_id: @farm.id }
  rescue ActiveRecord::RecordNotFound
    redirect_to new_cultivation_plan_path, alert: '栽培地域を選択してください。'
  end
  
  # Step 3: 作物選択
  def select_crop
    unless session_data[:farm_id]
      redirect_to new_cultivation_plan_path, alert: '最初からやり直してください。' and return
    end
    ...
    unless @farm_size
      redirect_to select_farm_size_cultivation_plans_path(farm_id: @farm.id), 
                  alert: '農場サイズを選択してください。' and return
    end
    
    @crops = Crop.reference.order(:name)
    session_data[:total_area] = @farm_size[:area_sqm]
    session[:cultivation_plan] = session_data
  rescue ActiveRecord::RecordNotFound
    redirect_to new_cultivation_plan_path, alert: '最初からやり直してください。'
  end
  ...
  def create
    ...
    if crops.empty?
      redirect_to select_crop_cultivation_plans_path, alert: '作物を1つ以上選択してください。' and return
    end
    ...
    if result.success?
      session[:cultivation_plan] = { plan_id: result.cultivation_plan.id }
      
      # 非同期で最適化実行
      OptimizeCultivationPlanJob.perform_later(result.cultivation_plan.id)
      
      redirect_to optimizing_cultivation_plans_path
    else
      redirect_to new_cultivation_plan_path, alert: "計画作成に失敗しました: #{result.errors.join(', ')}"
    end
  rescue ActiveRecord::RecordNotFound
    redirect_to new_cultivation_plan_path, alert: '最初からやり直してください。'
  end
  ...
  def optimizing
    ...
    # 完了している場合は結果画面へ
    redirect_to results_cultivation_plans_path if @cultivation_plan.status_completed?
  end
  ...
  def results
    ...
    # まだ完了していない場合は進捗画面へ
    redirect_to optimizing_cultivation_plans_path unless @cultivation_plan.status_completed?
  end
  ...
  def find_cultivation_plan
    ...
    unless plan_id
      redirect_to new_cultivation_plan_path, alert: '作付け計画が見つかりません。'
      return nil
    end

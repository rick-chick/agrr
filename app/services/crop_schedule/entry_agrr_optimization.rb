...
module CropSchedule
  ...
  class EntryAgrrOptimization
    ...
    # @return [CropSchedule::WindowService::Result] 成功時は eligible: true。失敗時も同型で eligible: false と理由のみ。
    ...
    def call
      ...
      Rails.logger.warn("[EntryAgrrOptimization] daemon: #{e.message}")
      failed_result(:daemon_unavailable)
    rescue Agrr::BaseGatewayV2::ExecutionError, AgrrService::CommandExecutionError => e
      Rails.logger.warn("[EntryAgrrOptimization] execution: #{e.class}: #{e.message}")
      failed_result(:execution_failed)
    rescue Agrr::BaseGatewayV2::ParseError, ArgumentError, Date::Error, TypeError, JSON::ParserError => e
      Rails.logger.warn("[EntryAgrrOptimization] parse/args: #{e.class}: #{e.message}")
      failed_result(:invalid_response)
    rescue StandardError => e
      # Crop#to_agrr_requirement の不足データ、その他想定外（作物1件で一覧全体を500にしない）
      Rails.logger.error("[EntryAgrrOptimization] #{e.class}: #{e.message}\n#{e.backtrace&.first(8)&.join("\n")}")
    ...
    # 最適化に渡す評価期間: 昨年6/1〜来年6/30（暦年ベース）と気象日次の積集合。

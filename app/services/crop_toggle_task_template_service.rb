...
# 作物詳細の「作業テンプレート」トグル（テンプレート作成/削除と手動ブループリントの追随）を集約する。
# HTML（Turbo / 通常）双方のコントローラから呼び出すための境界用サービス。
class CropToggleTaskTemplateService
  Result = Struct.new(:available_agricultural_tasks, :selected_task_ids, :task_schedule_blueprints, keyword_init: true)

  def call(crop:, agricultural_task:)
  ...
  def build_result(crop:)
    Result.new(
      available_agricultural_tasks: self.class.available_agricultural_tasks_for_crop(crop),
      selected_task_ids: self.class.selected_task_ids_for_crop(crop),

...
module Forms
  # 農場マスタ HTML 用（ActiveRecord をビューに渡さない）。param キーは `farm`。
  class FarmMasterForm
    include Forms::HtmlFormSupport

    ATTR_KEYS = %i[name latitude longitude region is_reference user_id].freeze

    attr_accessor(*ATTR_KEYS)
    attr_reader :id

    def self.model_name
      Domain::Shared::FormModelName.from_logical_name("Farm")
    end

    # @param snapshot [Domain::Farm::Dtos::FarmMasterFormSnapshot]
    # @return [Forms::FarmMasterForm]
    ...
    def to_ar_attributes
      {
        ...
        region: region,
        user_id: user_id
      }.compact.merge(is_reference.nil? ? {} : { is_reference: is_reference })
    end
  end
end

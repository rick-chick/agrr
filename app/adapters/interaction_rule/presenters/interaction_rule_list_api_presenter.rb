...
module Adapters
  module InteractionRule
    module Presenters
      class InteractionRuleListApiPresenter < Domain::InteractionRule::Ports::InteractionRuleListOutputPort
        def initialize(view:)
          @view = view
        end
        ...
        def on_failure(error_dto)
          if error_dto.is_a?(Domain::Shared::Policies::PolicyPermissionDenied)
            @view.render_response(
              json: { error: I18n.t("interaction_rules.flash.no_permission") },
              status: :forbidden
            )
            return
          end

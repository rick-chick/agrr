  ...
  module FieldCultivation
    module MockProgressRecords
      def generate_mock_progress_records(start_date, end_date, logger:)
        ...
        logger.info "🧪 [Adapters::FieldCultivation::MockProgressRecords] Generated #{records.length} records, GDD range: 0-#{records.last['cumulative_gdd']}"
        logger.info "🧪 [Adapters::FieldCultivation::MockProgressRecords] Stage distribution: #{records.group_by { |r| r['stage_name'] }.transform_values(&:count)}"

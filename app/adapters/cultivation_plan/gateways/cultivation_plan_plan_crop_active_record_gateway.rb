    ...
    module Gateways
      class CultivationPlanPlanCropActiveRecordGateway <
        ...
        def create_for_plan(attrs:)
          cultivation_plan = ::CultivationPlan.find(attrs.plan_id)
          crop = ::Crop.find(attrs.crop_id)

          plan_crop = cultivation_plan.cultivation_plan_crops.create!(
            crop: crop,
            name: attrs.name,
            variety: attrs.variety,
            area_per_unit: attrs.area_per_unit,
            revenue_per_area: attrs.revenue_per_area
          )

    ...
    module Gateways
      class CultivationPlanActiveRecordGateway < Domain::CultivationPlan::Gateways::CultivationPlanGateway
        ...
        def private_plan_optimizing_snapshot(plan_id:, user:)
          ...
          Domain::CultivationPlan::Dtos::PrivatePlanOptimizingSnapshot.new(
            id: plan.id,
          ...
          plans.each do |plan|
            plan.cultivation_plan_fields.each do |plan_field|
              ...
              unless fields_hash[field_name]
                fields_hash[field_name] = Domain::CultivationPlan::Dtos::PlanningScheduleField.new(
          ...
          plans.each do |plan|
            ...
            plan.field_cultivations.each do |field_cultivation|
              ...
              if plan.plan_year.nil? || field_cultivation.start_date.year == plan.plan_year
                cultivations << Domain::CultivationPlan::Dtos::PlanningScheduleCultivation.new(
        ...
        # 農場・期間で一括取得（N+1回避）。フィールド名でグループ化して返す。
        # @return [Hash{String => Array<PlanningScheduleCultivation>}] field_name => cultivation DTOs
        def planning_schedule_cultivations_for_farm(user:, farm_id:, period_start:, period_end:)
          ...
          # field_name => [PlanningScheduleCultivation]
          ...
          plans.each do |plan|
            ...
            plan.field_cultivations.each do |field_cultivation|
              next unless field_cultivation.start_date &&
                field_cultivation.completion_date &&
                field_cultivation.start_date <= period_end &&
                field_cultivation.completion_date >= period_start

              field_name = field_cultivation.cultivation_plan_field.name
              next if field_name.blank?

              if plan.plan_year.nil? || field_cultivation.start_date.year == plan.plan_year
                cultivations_by_field[field_name] ||= []
                cultivations_by_field[field_name] << Domain::CultivationPlan::Dtos::PlanningScheduleCultivation.new(
                  crop_name: field_cultivation.cultivation_plan_crop.name,
                  start_date: field_cultivation.start_date,
                  completion_date: field_cultivation.completion_date,
                  area: field_cultivation.area
          ...
          # 各フィールド内を開始日でソート
          cultivations_by_field.transform_values { |cults| cults.sort_by { |c| c.start_date } }
        end

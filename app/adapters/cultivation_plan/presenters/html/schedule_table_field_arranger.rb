...
# 計画スケジュール表の1ほ場の作付配置を決定する（HTML 表示用）。
# T-034: app/services から lib/presenters/html/plans へ移管。
# アルゴリズム:
...
class Adapters::CultivationPlan::Presenters::Html::ScheduleTableFieldArranger
  ...
  def self.arrange(cultivations:, periods:)
    ...
    cultivations_with_periods = cultivations.map do |cultivation|
      periods_for_cultivation = periods.select do |period|
        # 作付が期間と重なるかどうか（DTO/Hash両方対応）
        start_date = cultivation.respond_to?(:start_date) ? cultivation.start_date : cultivation[:start_date]
        completion_date = cultivation.respond_to?(:completion_date) ? cultivation.completion_date : cultivation[:completion_date]
        start_date <= period[:end_date] && completion_date >= period[:start_date]
      end
    ...
    pending_cultivations.each do |pending|
      ...
      if overlaps_with_pending
        raise OverlappingError, "保留の作付同士が重なっています: #{pending[:cultivation].respond_to?(:crop_name) ? pending[:cultivation].crop_name : pending[:cultivation][:crop_name]}"
      end

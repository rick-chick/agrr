...
module Adapters
  module AgriculturalTask
    module Presenters
      class AgriculturalTaskUpdateApiPresenter < Domain::AgriculturalTask::Ports::AgriculturalTaskUpdateOutputPort
        def initialize(view:)
          @view = view
        end

        def on_success(task)
          json = entity_to_json(task)
          @view.render_response(json: json, status: :ok)
        end

        def on_failure(error_dto)
          if error_dto.is_a?(Domain::Shared::Policies::PolicyPermissionDenied)
            @view.render_response(json: { error: I18n.t("agricultural_tasks.flash.no_permission") }, status: :forbidden)
            return
          end

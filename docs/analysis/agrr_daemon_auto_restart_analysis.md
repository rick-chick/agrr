...
**実装箇所**: `Agrr::DaemonClient` のコマンド実行経路（`app/gateways/agrr/daemon_client.rb`。本文中の AgrrService は歴史的名称）
...
#### Agrr::DaemonClient（旧称 AgrrService、`app/gateways/agrr/daemon_client.rb`）
...
- `execute_command` メソッド（17-82行目）で `Agrr::DaemonClient` を使用（インスタンスは `@agrr_service` に保持）
...
- この例外は `Agrr::DaemonClient` のコマンド実行経路で捕捉される
...
**発生箇所**: `Agrr::DaemonClient` の各メソッド
- 例: `Agrr::DaemonClient#weather`
...
- この例外は `Agrr::DaemonClient` のコマンド実行経路で捕捉されて `CommandExecutionError` になる可能性

### パターン3: コマンド実行時のエラー
**発生箇所**: `Agrr::DaemonClient` のコマンド実行処理
- エラー: `CommandExecutionError`
- 発生条件: agrr_clientが非ゼロのexit codeを返し、かつ有効なJSONが含まれていない場合

## 実装箇所: `Agrr::DaemonClient` のコマンド実行

**決定**: `Agrr::DaemonClient` のコマンド実行経路で自動起動・リトライ処理を実装する。
...
4. **既存コードへの影響が少ない**: デーモンクライアントに集約し、Gateway 層や Controller 層への波及を抑える
...
### 実装対象
- `app/gateways/agrr/daemon_client.rb` - **実装箇所**（コマンド実行・リトライ）

# Layer rules (L2 — detailed normative spec)

**Status:** canonical detail for layer boundaries. **L1 codemap:** [`ARCHITECTURE.md`](../../ARCHITECTURE.md).

> **Runtime (P7+):** Production HTTP/WS/auth run in `crates/agrr-server` with `agrr-domain` and `agrr-adapters-*`. Ruby `lib/domain` and `app/controllers/api` are **removed** (historical names below). Map: `lib/domain` → `crates/agrr-domain`, `app/adapters` → `crates/agrr-adapters-*`, Rails controller edge → Axum handlers and `crates/agrr-server/src/composition.rs`.

## Historical path note (Ruby era)

The sections below retain Ruby path examples where they clarify **layer rules** the Rust stack follows. For current code paths, use `crates/agrr-domain` and `crates/agrr-server` only.
...
Normative list for **new** adapter gateway public methods. Enforce via R4 contract tests and code review (Ruby `test/architecture/gateway_public_method_naming_test.rb` removed in P8).
...
This section defines the normative rules for new code. Positive rules are stated first; **prohibited patterns** (**❌**) show common violations (architecture gate references these as prohibitions 1–39). Where common community patterns, prevailing pragmatism, or existing project conventions conflict with the rules below, this file takes precedence.

...
### M5. ヒンディー語（in）ユーザーのレポート導線が日本語版に飛ぶ — **対応済み** (#735)

`frontend/src/app/components/shared/navbar/navbar.component.ts` — `reportUrl` を `lang === 'ja' ? '/research/' : '/research/en/'` に変更。`in` ロケールは英語版 research へフォールバック。

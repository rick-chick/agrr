...
1. ドメインゲートウェイ経由（`find_by_*` 形式）でキャッシュ済み予測を読む（実装は [`weather_prediction_interactor.rb`](../../lib/domain/weather_data/interactors/weather_prediction_interactor.rb) を参照）。

...
新規 UI は `frontend/` のみ。既存 Rails View は本 ADR 末尾「撤去ロードマップ」に従い順次置換後、依存を削除する。

## Consequences

撤去ロードマップ完了までルート JS バンドルと Gem（`turbo-rails`、`stimulus-rails`）が併存する。
...
## 撤去ロードマップ（旧 `docs/planning/hotwire_removal_plan.md` を統合）
...
| Gem | `Gemfile`: `turbo-rails`, `stimulus-rails`, `jsbundling-rails` |
| ビルド出力 | `app/assets/builds/application.js` に Turbo バンドル |
...
- Gantt / optimizing / task schedule HTML・マスタ AI 等の Angular 移行完了を確認したうえで Hotwire 撤去タスクを起票する。

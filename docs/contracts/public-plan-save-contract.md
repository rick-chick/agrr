...
- [本契約（public-plan-save-contract.md）](./public-plan-save-contract.md) の API 失敗時は **`{ "success": false, "error": string }`**。400（`plan_id` 欠落）、404（計画なし）、422（保存処理の業務失敗）、500（想定外）を Presenter が付与する。

...
**機能概要**: 作付計画詳細（Angular）で、栽培帯選択時に気温・累積GDD・要求GDD・成長ステージを可視化する分析チャートを表示し、意思決定を支援する。真実の源は**本契約本文・この契約に紐づくテスト・API のレスポンススキーマ**とし、既存 HTML 実装との一致は要件としない（差分は契約またはテストの更新で明示する）。  
**ステータス**: draft

## ビジネス要件

- 作付計画をクリックした（Angular 上のガントチャート内の栽培帯を選択した）ユーザーに対し、日別気温、累積GDD、要求GDD、成長ステージを可視化したチャートを提示する。
...
- バックエンド上の `FieldCultivation` に `start_date` と `completion_date` が設定済みであること（本 API の前提）。
...
### 例外フロー
- **Exc-1**: `field_cultivation` が存在しない、またはユーザーがアクセス権を持たない場合 → API は 404 を返し Angular 側は `NotFound` 相当のエラーメッセージを表示。
- **Exc-2**: `start_date` もしくは `completion_date` が欠けている場合 → API は 400 を返し `controllers.field_cultivations.errors.no_cultivation_period` を返す。
- **Exc-3**: 天気情報が存在しない（`farm.weather_location` が nil）の場合 → API は 404。
...
### Phase 3: テスト実装
- [ ] `FieldCultivationClimateDataInteractor` と Controller の統合テストを追加（本契約の成功・エラーケースを押さえる）。
...
- [ ] エラーメッセージや閉じる操作など UX が破綻なく利用できる品質になっているか（期待は本契約・関連テスト・プロダクトガイドラインに従う）。

### 技術要件
- [ ] API の `success`, `debug_info`, `progress_result` などのキー名が**本契約の Success Response およびエラーペイロード**と一致し、Angular 型定義がそれに追従しているか。

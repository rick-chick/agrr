...
| 「日程をずらす」 | 予定の編集 | 既存 timeline（ドラッグ）/ 日付編集 |
...
| パス | 状態 |
|---|---|
| `frontend/src/app/components/plans/plan-task-schedule.component.ts` | 予定表示ページ（`/plans/:id/task_schedule` 相当のルートは `routes/plans.routes.ts`）。表示のみ |
| `frontend/src/app/components/plans/task-schedule-timeline.component.ts` | 圃場×カテゴリ（general/fertilizer）の列表示 + cdkDragDrop。**ドラッグはローカル配列の入れ替えだけで永続化されていない**（API 呼び出しなし） |
...
日付中心のタスクリスト。plan 中心のガント（既存 timeline）とは役割を分ける。
...
- 日付変更: v1 は `[⋮]` → 日付ピッカーのみ。既存 timeline の cdkDragDrop 永続化は**本計画のスコープ外**（予定 update API の配線が BE 側で別途必要なため。ドラッグが永続化されない現状は誤解を招くので、F3 時点でドラッグ無効化 or 「保存されません」注記のどちらかを入れること — 判断に迷ったらユーザーに確認）
- 手動予定追加はスコープ外（§6）
...
**変更**: skip 済み行の非表示 + トグル。timeline のドラッグ無効化 or 注記(§3.4)。
...
- timeline ドラッグ＆ドロップの永続化（予定 update API の配線が先）

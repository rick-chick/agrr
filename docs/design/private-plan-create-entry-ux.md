...
- workbench payload（`GET /api/v1/plans/cultivation_plans/{id}/data`）に `available_crops`（マスタ作物行）が含まれ、ガント UI（`gantt-chart.component.ts` / `gantt-plan-api.gateway.ts`）に add_crop / add_field / remove_field / adjust の操作系が実装済み

...
- **次に先頭で固定する修正単位（未着手）**: 通し走査の続き — **`DeletionUndo::HtmlMasterScheduleInvoker` に `record:` を渡している他マスタ**（`AgriculturalTasksController` / `PestsController` / `PesticidesController` / `FertilizesController`）を `resource_type` / `resource_id` に寄せる。併せて `Farms::WeatherDataController` の AR 残りを意味読みで backlog 化。

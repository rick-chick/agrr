...
- [ ] **`lib/domain` の `Api` / `ApiV1` 型名の横断棚卸し（`ApiWeather` BC を除く）— 残サブバッチ** — 禁止 **4** @ Interactors・ports・dtos
  - **次サブバッチ（先頭固定）**: `lib/domain/file_blob/gateways/api_file_blob_gateway.rb` と `lib/adapters/stored_blobs/gateways/api_file_blob_active_record_gateway.rb` の **`Api` 語を IF／具象クラス名から除去**、`CompositionRoot#api_file_blob_gateway` を `file_blob_gateway` に改名（`FileBlob*` Interactor 注入）
...
- **次に先頭で固定する修正単位（未着手）**: 上記 `[ ]` の **先頭**（`ApiFileBlobGateway` / `api_file_blob_gateway` 改名）をワークフロー セクション0 のスコープに固定する。`[ ]` が尽きたあとにのみ、空 backlog の通し走査を再実行する。

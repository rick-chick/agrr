...
2. **Application edge 禁止3（`rescue` 主スイッチ）** — `app/controllers/api` に AgrrService・JSON パース・`PolicyPermissionDenied` 相当の広い `rescue` が残るコントローラを、Interactor + Presenter + Gateway 正規化で個別に排除する（代表: `api/v1/pests_controller.rb`, `api/v1/crops_controller.rb`, `api/v1/fertilizes_controller.rb`, `api/weather_controller.rb`, `api/v1/cultivation_plan_rest_base_controller.rb`, `api/v1/public_plans/entry_schedule_controller.rb`, `auth_controller.rb` 等。一覧は禁止3の意味読みで都度確定すること）。

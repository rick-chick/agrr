...
1. **Angular（components → usecase）** — `*.component.ts` が `adapters/` を直接 import している残件を、usecase 側の `*.providers.ts`（またはルート配線）に寄せる。**先頭（辞書順）**: `frontend/src/app/components/masters/agricultural-tasks/agricultural-task-create.component.ts`（`agricultural-task-list` は 2026-05-06 対応済み）。

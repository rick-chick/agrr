...
- **解消済み（2026-05-08）**: `**retrieve plan data` の `ApiPlanData*` 除去** — `ApiPlanDataOutputPort`→`RetrieveCultivationPlanOutputPort`、`ApiPlanDataPresenter`→`RetrieveCultivationPlanPresenter`（`#data` / `RetrieveCultivationPlanInteractor`）。禁止 **4**。

- [ ] **`lib/domain` の `Api` / `ApiV1` 型名の横断棚卸し（`ApiWeather` BC を除く）— 残サブバッチ** — 禁止 **4** @ Interactors・ports・dtos
  - **次サブバッチ（先頭固定）**: `lib/domain/cultivation_plan/ports/api_private_plan_create_output_port.rb`、`lib/domain/cultivation_plan/dtos/api_private_plan_create_{input,success,failure}_dto.rb`、`lib/presenters/api/plans/api_private_plan_create_presenter.rb` の **チャネル語 `Api` をクラス名・ファイル名から除去**（`PrivatePlanInitializeFromSelectionInteractor` 経路・`Api::V1::PlansController#create`）
...
- **次に先頭で固定する修正単位（未着手）**: 上記 `[ ]` の **先頭**（`**cultivation_plan` の add_crop 用 Output Port / Presenter の `Api` 語除去**）をワークフロー セクション0 のスコープに固定する。`[ ]` が尽きたあとにのみ、空 backlog の通し走査を再実行する。

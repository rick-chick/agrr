...
- [ ] **CultivationPlan REST 共通アクションをエッジ最薄化** — `Api::V1::CultivationPlanRestBaseController` がアクション本体・Gateway 組み立て・ログ整形を抱える。各アクション（`add_crop` / `add_field` / `remove_field` / `data` / `adjust`）を「入力 DTO + Interactor 呼び出し + Presenter 注入」だけに縮め、Gateway 構築は `CompositionRoot` へ集約する。@ `app/controllers/api/v1/cultivation_plan_rest_base_controller.rb`, `lib/composition_root.rb` — Application edge 1 / Interactors 3
- [ ] **HTML 計画ウィザードの判断をドメインへ** — `CultivationPlanHtmlBaseController#handle_optimizing` / `#find_cultivation_plan` がステータス判定・リダイレクト先選択をエッジで行う。Interactor が出力 DTO で「完了済み・未完了・存在しない」を返し、Presenter が `redirect_to` を担う形へ。@ `app/controllers/cultivation_plan_html_base_controller.rb` — Application edge 1 / Interactors 1

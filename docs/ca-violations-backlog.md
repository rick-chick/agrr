...
1. **Application edge 1 / Sideways escape — `app/controllers/concerns/**` の `ActiveSupport::Concern` によるオーケストレーション共有**
   ...
   - **残り（優先）**: `agrr_optimization#adjust_with_db_weather` の主導線。その他 concern（`cultivation_plan_api` 等）は別サブ項目として継続。
...
- **`agrr_optimization` — `save_adjusted_result` の Agrr adjust 結果永続化**（2026-05-06）: FieldCultivation 差分 upsert/insert・TaskSchedule の null 化・未使用 plan crop 削除・計画サマリ更新を `SaveAdjustedAgrrResultGateway`（IF）/`SaveAdjustedAgrrResultActiveRecordGateway`（AR）へ移動。`clock`・`LoggerGateway` を `CompositionRoot` から注入。`AgrrOptimization#save_adjusted_result` は `plan_id` 委譲のみ。項目 1 の残り: `adjust_with_db_weather` 主導線・他 controller concerns。

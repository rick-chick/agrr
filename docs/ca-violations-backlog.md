...
- `**lib/domain` の `Api` / `ApiV1` 型名の横断棚卸し（`ApiWeather` BC を除く）— 残サブバッチ** — 禁止 **4** @ Interactors・ports・dtos・presenters
  - **次サブバッチ（先頭固定）**: `lib/presenters/api/plans/api_v1_private_plans_list_presenter.rb` の `**ApiV1PrivatePlansListPresenter` を `PrivateOwnedPlansListPresenter` に改名**（ファイル名含む・`PlansController#index`）
...
- **次に先頭で固定する修正単位（未着手）**: 上記 `[ ]` の **先頭**（`ApiV1PrivatePlansListPresenter` 改名）をワークフロー セクション0 のスコープに固定する。`[ ]` が尽きたあとにのみ、空 backlog の通し走査を再実行する。

...
- **次に先頭で固定する修正単位（未着手）**: 通し走査の続き。**HTML** `CropsController#destroy` の **format.json**（`DeletionUndo::HtmlMasterScheduleInvoker` への `record: @crop`）など、`set_crop` 由来以外の AR 直渡しをゲートウェイ／Interactor 経路に寄せるか、`DeletionUndo` 契約を DTO 化するかを決めてから着手。併せて `Farms::WeatherDataController` 等の grep 候補を意味読みで backlog 化。

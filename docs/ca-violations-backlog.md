...
- （**解消済み・2026-05-12**）`Domain::Pesticide::Dtos::AuthorizedPesticideLoadedDto` の `persisted_pesticide` を `master_form_snapshot`（`PesticideMasterFormSnapshot`）に置換。`PesticideActiveRecordGateway#find_authorized_pesticide_loaded_bundle!` で `PesticideMasterFormSnapshotMapper.from_record`。`lib/presenters/html/pesticide/pesticide_load_for_view_html_presenter.rb`（死コード）を削除。

...
ドメイン BC の実装順・パリティは完了（[`TRACKING.yaml`](../lib-domain-rust/TRACKING.yaml)）。
...
| フェーズ | 名称 | スタック上の成果 |
|----------|------|------------------|
| **P0–P5** | ドメイン（[`lib-domain-rust`](../lib-domain-rust/)） | **完了**（2026-05-29: TRACKING 19/19 `phase: done`）— `agrr-domain` + R0/R1/R2 パリティ |
...
| 新規 `agrr-adapters-*` / `agrr-server` クレート | 本書 + R4 契約 |
...
- [`lib-domain-rust/README.md`](../lib-domain-rust/README.md) — ドメイン移行（完了）
- [`ARCHITECTURE.md`](../../../ARCHITECTURE.md) — Gateway boundary

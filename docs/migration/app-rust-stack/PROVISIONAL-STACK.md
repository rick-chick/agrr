...
WebSocket ワイヤ・ジョブチェーン・P6 マイグレーション所有・R4 複製元・削除 undo（Angular 化）・P7 **refinery**・ジョブ耐障害（`:async` 受容）・**OAuth コールバック URL**（現行パス維持）は **本書に確定**。**ユーザー添付は未使用のため P6 対象外**（2026-05-29 削除）。ストラングラー配線のみ **別 ADR**（下記「未確定」）。ブロッカー整理は [`BLOCKERS-RESPONSE.md`](./BLOCKERS-RESPONSE.md)。
...
| 移行期 | Rust / Rails 二進または同一 LB 下の二サービス（ルートで振分） |
...
| フェーズ | 名称 | スタック上の成果 |
|----------|------|------------------|
| **P0–P5** | ドメイン（[`PROGRAM.md`](../lib-domain-rust/PROGRAM.md)） | `agrr-domain` + R0/R1/R2 パリティ |
...
1. **ストラングラー配線** — 同一 Cloud Run 内プロキシ vs 二サービス + URL map（OAuth は案 A のため、**同一ホスト + パス振分**なら Console 変更なしで切替可能）。回答の正: [`BLOCKERS-RESPONSE.md`](./BLOCKERS-RESPONSE.md)。

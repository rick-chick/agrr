...
親 issue: #222。Backend 実装: #223（PR #227–#228）、登録 UI: #224（PR #229）、実績履歴閲覧 UI: #225（PR #232）、削除 undo・ストレージ整合性: #226（PR #233）。レイアウト確定: #234。
...
| 実績履歴リスト | 対応済み（#225）: 最大 3 枚サムネイル + ライトボックス |
| 編集シート | 登録シートと同一コンポーネントで対応 |
| 今日の作業 | v1 非スコープ |

#### 8.1 サムネイルレイアウト（v1 標準・#234 で確定）

**採用: 横並び（案 A）** — リスト・登録シートとも、1 記録あたり最大 3 枚を 1 行の flex 行（`display: flex; flex-wrap: wrap`）で表示する。

| 案 | 判定 | 理由 |
|----|------|------|
| **横並び（案 A）** | **採用** | スマホ撮影の landscape 写真と視線（左→右）が一致。複数枚の比較が容易。テキスト行下の 1 段写真帯で縦スペースを抑える |
| 縦積み | 不採用 | landscape 写真の特性を活かせず、リスト行の縦幅・スクロール負荷が増える |
| 1 枚 + 「+N」（案 B） | v1 見送り | 3 枚横並びでモバイル幅の折り返しに問題がなければ不要。折り返しで行高が不安定になった場合に再検討 |

**アスペクト比**: 正方形ではなく **横長 4:3**（`aspect-ratio: 4 / 3`）。`object-fit: cover` による上下クロップを減らし、圃場・作物の landscape 写真の内容を伝えやすくする。

| 画面 | クラス | 幅（基準） | 比率 |
|------|--------|-----------|------|
| 実績履歴リスト | `.plan-work-records__photo-thumb` | `4rem` | 4:3 |
| 登録・編集シート | `.work-record-sheet__photo-thumb` | `4.5rem` | 4:3 |

定数: `frontend/src/app/domain/plans/work-record-photo.constants.ts`（`WORK_RECORD_PHOTO_THUMB_ASPECT_RATIO` 等）。

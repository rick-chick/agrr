...
| **回答** | **P6 着手の lib/domain ブロッカーはなし**（[`README.md`](../lib-domain-rust/README.md) と一致）。Ruby adapter §P4（field_cultivation / cultivation_plan read）は 2026-05-29 対応済み。残るのは `agrr-domain` パリティ・§5 `agrr-server` 未作成・BC 単位の R4 |
| P6 ブロック | **いいえ**（プログラム全体。§3・R4 は **切替対象 BC** ごと） |

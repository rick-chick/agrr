...
GitHub イベント
  ...
  → Delivery Agent
  → gh 観測 → 判断（実装 / 修正 / merge / close / 待ち）
```

**機械は判断しない。** reconcile も webhook 再送まで。merge / close / obsolete は Agent が毎 run 観測して決める。**判断基準（正本）**: [JUDGMENT-CRITERIA.md](../../.cursor/skills/automation-authoring/references/JUDGMENT-CRITERIA.md)。設計原則: [PRINCIPLES.md §二層分離](../../.cursor/skills/automation-authoring/references/PRINCIPLES.md#二層分離正本)。

**リトライ**: `issue-worker-retry-dispatch.yml` / `pr-merge-worker-retry-dispatch.yml` が 15 分ごとに滞留へ webhook 再送。primary が `cancelled` のときも再送。

**pr-agent-prep**（機械のみ）: `closingIssuesReferences` ありの Draft PR を直列 `gh pr ready`。**判断ラベルは付けない。**
...
**依存判断**: 機械層（dispatch / reconcile）は **依存を判定しない**。`agent-ready` イベントで webhook を送り、Delivery Agent が `gh issue view` で本文・依存を読んで実装・待ち（コメントのみ）・close を決める。本文・コメント・依存ラベルのパースは機械層禁止。

...
**レガシー実装注意**: dispatch lib に `conflict` / `ci_fix` / `pr_review` 等の内部ゲート名が残存している。これは webhook 再送の実装詳細であり、Agent の判断根拠ではない（[JUDGMENT-CRITERIA §6](../../.cursor/skills/automation-authoring/references/JUDGMENT-CRITERIA.md#6-レガシーコードに残存廃止予定)）。

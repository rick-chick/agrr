...
`agricultural_task`, `api_keys`, `auth`, `backdoor`, `contact_messages`, `crop`, `cultivation_plan`, `deletion_undo`, `farm`, `fertilize`, `field`, `field_cultivation`, `interaction_rule`, `internal_jobs`, `pest`, `pesticide`, `public_plan`, `user`, `weather_data`, `work_record`, `shared`。各文脈は `entities/`, `dtos/`, `gateways/`(interface), `interactors/`, `ports/`。

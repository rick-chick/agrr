...
`agricultural_task`, `api_keys`, `auth`, `backdoor`, `contact_messages`, `crop`, `cultivation_plan`, `deletion_undo`, `farm`, `fertilize`, `field`, `field_cultivation`, `file_blob`, `interaction_rule`, `internal_jobs`, `pest`, `pesticide`, `planning_schedule`, `public_plan`, `user`, `weather_data`, `shared`。各文脈は `entities/`, `dtos/`, `gateways/`(interface), `interactors/`, `ports/`。

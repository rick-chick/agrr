...
1. **`ARCHITECTURE.md`** — レイヤ境界・禁止事項（`## What we require`、`## Prohibited practices` 1–30）の**最上位ソース**。「既存もそう」「テストが通った」は規約の根拠にならない。
2. `docs/contracts/*.md` と該当テスト
...
- **Presenter**（`lib/presenters/{api,html}/<resource>/`）は output port を実装し、HTTP 形に写すだけ。`CompositionRoot` / `find_model` / gateway 取得・認可・本質バリデーションを置かない。**新規 presenter は `app/presenters/` ではなく `lib/presenters/` に置く**。
- **Gateway** は狭い永続化／I/O。表現・認可・ユースケース分岐を含めない（"画面ごとの blob を返す Gateway" は境界違反。Interactor 側で組み立てる）。
- **HTML マスタ CRUD も**同じドメインを通る（presenters は `lib/presenters/html/`）。同一ユースケースは HTML と API で **同じ Interactor**。`*HtmlInteractor` / `*ApiInteractor` の分割は禁止。

### 主要なドメイン文脈（`lib/domain/<context>/`）

`agricultural_task`, `contact_messages`, `crop`, `cultivation_plan`, `deletion_undo`, `farm`, `fertilize`, `field`, `field_cultivation`, `interaction_rule`, `pest`, `pesticide`, `public_plan`, `weather_data`, `shared`, `logger`。各文脈は `entities/`, `dtos/`, `gateways/`(interface), `interactors/`, `ports/`。
...
`app/gateways/agrr/` が CLI / デーモンプロトコルをカプセル化（最適化、気象、進捗）。テストは `test/gateways/agrr/`。
...
- 強制は ActiveRecord の validations（モデル境界）
...
```bash
.cursor/skills/deploy-server/scripts/gcp-deploy.sh    # Production（agrr.net）
.cursor/skills/deploy-frontend/scripts/gcp-frontend-deploy.sh  # Frontend → GCS + Cloud CDN
```

### Git 操作の制約

`git checkout` / `git switch` / `git reset` など**ブランチ／作業ツリー状態を変える**操作は、ユーザーの明示許可なしには実行しない。`git add` / `git commit` / `git status` / `git diff` / `git log` は可。`git push` や force 系はユーザー明示時のみ。
...
2. **セクション4 — ARCHITECTURE.md ゲート（1 回目・2 回目）**: `.cursor/rules/ca-violation-fix-architecture-gate.mdc` の手順で禁止 1–30 と照合・記録出力する。Rails / `frontend/` のみの差分でも同一フォーマット。
...
- [docs/contracts/](docs/contracts/) — API・機能契約（contract-first）

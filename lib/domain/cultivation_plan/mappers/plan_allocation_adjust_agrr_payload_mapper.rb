    ...
    module Mappers
      ...
      module PlanAllocationAdjustAgrrPayloadMapper
        ...
        def to_current_allocation(snapshot:, exclude_ids: [], logger:)
          field_cultivation_count = snapshot.field_source_rows.sum { |row| row.cultivations.size }
          ...
          AgrrAdjustAllocationRowMapper.build_current_allocation(
            ...
            field_rows: snapshot.field_source_rows,
        ...
        def to_fields_config(snapshot:)
          plan_fields = snapshot.plan_fields.map do |field|
            {
              id: field.id.to_s,
              name: field.name,
              area: field.area,
        ...
        def to_crops_config(snapshot:, logger:)
          entries = snapshot.plan_crop_entries.map do |entry|
            {
        ...
        def to_interaction_rules(snapshot:, random_hex:)
          crop_groups = {}
          snapshot.plan_crop_entries.each do |entry|

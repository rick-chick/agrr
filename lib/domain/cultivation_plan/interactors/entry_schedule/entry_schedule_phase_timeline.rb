    ...
    module Interactors
      module EntrySchedule
        ...
        class EntrySchedulePhaseTimeline
          ...
          # @param crop [Crop]
          # @param result [WindowService::Result]
          ...
          def rough_timeline(crop, result)
            ...
            by_month.keys.sort.map do |ym|
              ...
              {
                month: ym,
                summary: t("api.entry_schedule.timeline.month_summary", labels: labels.join("・"))
              }
            end
          end
          ...
          def schedule_flow_summary(crop, result)
            ...
            t("api.entry_schedule.flow.summary", phases: parts.join("→"))
          end
          ...
          def agrr_ratio_phase_segments(result)
            ...
            unless result.eligible && w
              return [
                phase_base(:sowing).merge(empty_reason: t("api.entry_schedule.phase.empty.ineligible")),
                phase_base(:nursery).merge(empty_reason: t("api.entry_schedule.phase.empty.ineligible")),
                phase_base(:transplant).merge(empty_reason: t("api.entry_schedule.phase.empty.ineligible")),
                phase_base(:harvest).merge(empty_reason: t("api.entry_schedule.phase.empty.ineligible"))
              ]
            end
          ...
          def phase_base(key)
            {
              phase_key: key.to_s,
              label: t("api.entry_schedule.phase.label.#{key}"),
              start_date: nil,
              end_date: nil,
              empty_reason: nil
            }

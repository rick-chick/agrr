...
module Domain
  module WeatherData
    module Interactors
      class InternalFarmWeatherDataListInteractor
        ...
        def call(input_dto)
          r = @gateway.weather_data_list_snapshot(farm_id: input_dto.farm_id)
          if r.farm_not_found?
          ...
          elsif r.weather_location_not_found?
            @output_port.on_failure(
              Dtos::InternalWeatherFetchFailure.new(
                message: @translator.t("api.errors.common.weather_location_not_found"),
                http_status: :not_found
              )
            )
          elsif r.ok?

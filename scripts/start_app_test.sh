...
if [ "${USE_AGRR_DAEMON}" = "true" ]; then
    echo "=== Starting Rails Application with Litestream + agrr daemon (TEST) ==="
else
    echo "=== Starting Rails Application with Litestream (TEST) ==="
fi
...
if litestream restore -if-replica-exists -config /etc/litestream.test.yml /tmp/test.sqlite3; then
...
litestream replicate -config /etc/litestream.test.yml &
LITESTREAM_PID=$!

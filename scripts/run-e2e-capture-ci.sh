#!/usr/bin/env bash
# CI / local: Playwright e2e:capture-for-agent against docker compose dev Rust stack.
# Prerequisites: Docker, Node 20+, npm ci in frontend/.
#
# Usage:
#   scripts/run-e2e-capture-ci.sh           # full run (route × locale PNGs)
#   scripts/run-e2e-capture-ci.sh --dry-run # validate files only
set -euo pipefail
...
WAIT_SECS="${E2E_CI_HEALTH_WAIT_SECS:-240}"

if [[ "${1:-}" == "--dry-run" ]]; then
  test -f .github/workflows/frontend-e2e-capture.yml
  test -x scripts/run-e2e-capture-ci.sh || test -f scripts/run-e2e-capture-ci.sh
  test -f docker-compose.e2e-ci.yml
  test -f frontend/e2e/agent-review/verify-capture-complete.mjs
  exit 0
fi
...
echo "==> Running e2e:capture-for-agent (E2E_CAPTURE_DEV_SESSION=1 E2E_STRANGLER=1)"
export E2E_CAPTURE_DEV_SESSION=1
export E2E_STRANGLER=1
npm run e2e:capture-for-agent

echo "==> verify-capture-complete passed; PNGs under frontend/e2e/agent-review/out/"
echo "==> e2e:capture-for-agent GREEN"

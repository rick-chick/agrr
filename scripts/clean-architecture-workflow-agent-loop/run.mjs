...
1. セクション0: docs/ca-violations-backlog.md を読む。項目があるなら **先頭の修正単位** にスコープを固定する（ユーザーに順番を聞かない）。
   backlog が無い／中身から判断して「修正単位」が空なら、SKILL の「全体スキャン」を **全量** で実施し、結果を backlog に書くか、違反ゼロなら backlog を削除して外側ループ終了とする（agent-operational-canonical.md の Backlog ファイル運用）。
...
3. セクション6: 意図したスコープだけ git commit する。docs/ca-violations-backlog.md を更新する（解消した単位の削除・新規違反の追記）。残件がなくなったら全量再スキャンし、違反ゼロなら **backlog ファイルを削除** して外側ループ完了とする。

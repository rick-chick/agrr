...
| artifact | `frontend/e2e/agent-review/out/*.png` + `agent-review-bundle.json`（保持 14 日。57 ルート × 3 言語 ≈ 171 枚・合計数十 MB 程度） |
| bundle runId 整合 | キャプチャ後 CI が `stamp-review` → `evidence:check:enforce` を実行。repo 追跡の `agent-review-bundle.json` / `visual-review-results.md` の **captureRunId** はマニフェスト変更 PR（例 #713）で手動同期する。週次 artifact の `runId` と repo の bundle がずれたら再キャプチャ PR を起票する |

...
| 9 | `public-plans/new` | `public-plans_new.ja.png` | `public-plans_new.en.png` | `public-plans_new.in.png` | OK | OK | **#738 受け入れ**: #172 `localizePublicPlanReferenceFarmName` + `public_plans.reference_farms.*`（`public-plan-reference-farm-name.spec.ts` GREEN）。in「Punjab」は地名（マスタデータ） |
...
| 13 | `public-plans/select-farm-size` | `public-plans_select-farm-size.ja.png` | `public-plans_select-farm-size.en.png` | `public-plans_select-farm-size.in.png` | OK | OK | **#738 受け入れ**: #172 同根因。`localizePublicPlanReferenceFarmName`（`public-plan-create.component.spec.ts` GREEN） |
...
| 16 | `agricultural_tasks/:id` | `agricultural_tasks_id.ja.png` | `agricultural_tasks_id.en.png` | `agricultural_tasks_id.in.png` | OK | OK | **#738 受け入れ**: #170 修正済。`hours_suffix` 翻訳（`agricultural-task-detail.component.spec.ts` GREEN） |
...
| 28 | `farms/:id` | `farms_id.ja.png` | `farms_id.en.png` | `farms_id.in.png` | OK | OK | **#738 受け入れ**: #673 修正済 + farm-list 同パターン。`farms.form.region_*`（`farm-detail.component.spec.ts`・`farm-list.component.spec.ts`・`masters-detail-region-locale.catalog.spec.ts` GREEN） |
...
| 36 | `interaction_rules/:id` | `interaction_rules_id.ja.png` | `interaction_rules_id.en.png` | `interaction_rules_id.in.png` | OK | OK | **#738 受け入れ**: #171 修正済。`interaction_rules.show.region` / `form.region_*`（`interaction-rule-detail.component.spec.ts` GREEN） |
...
| 53 | `plans/new` | `plans_new.ja.png` | `plans_new.en.png` | `plans_new.in.png` | OK | OK | **#738 受け入れ**: #636 後 3 言語 subtitle 整合（農場選択のみ・年言及なし）。`plans-new-locale.catalog.spec.ts` GREEN |
...
2. **#9–13 public-plans** — **#738 受け入れ**: #172 `localizePublicPlanReferenceFarmName` で mojibake 解消。in「Punjab」は地名（データ層）。**残**: results は 404 二重表示（#11・本 issue スコープ外）。
3. **#10 public-plans/optimizing** — 気象取得失敗エラー（planId=1 のデータ/API 要因）。
4. **#16 agricultural_tasks/:id** — **#738 受け入れ済み**（#170）。`hours_suffix` 翻訳（component spec GREEN）。
5. **#25 farms/:id** — **#738 受け入れ済み**（#673 + farm-list）。`farms.form.region_*` 翻訳表示。
6. **#29–30 fertilizes/:id** — **#658 解消**: E2E Baseline 正常表示。エラー回復 UI は spec 検証済み。
7. **#33 interaction_rules/:id** — **#738 受け入れ済み**（#171）。region ラベル・値とも翻訳。
...
10. **#50 plans/new** — **#738 受け入れ済み**（#636）。3 言語 subtitle 整合（catalog spec GREEN）。
...
**i18n**: P0/P1 系（about・privacy/terms・task_schedule・マスタ en ラベル）は **大幅改善**。**#658**: fertilizes/pesticides detail+edit の HTTP エラー英語露出・`title_default` 生キーは **解消**。**#738**: hours_suffix・interaction_rules.show.region・public-plans mojibake・farms 地域コード（detail + farm-list）・plans/new subtitle は **受け入れ済み**（catalog/component spec）。**残 i18n 要確認 2 件**（#11 public-plans/results 404、#20 Reference Crop 連結）— データ層・別 issue 候補。

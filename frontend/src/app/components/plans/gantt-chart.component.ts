      ...
      <div class="gantt-action-bar">
        @if (isMobileLayout) {
          <div class="gantt-action-bar__primary-actions">
            <button
              class="btn btn-primary btn-sm gantt-action-bar__crop-primary"
              type="button"
        ...
        <div class="gantt-range-controls">
          <button
            class="btn btn-secondary btn-sm"
            [class.gantt-range-button--icon]="isMobileLayout"
            type="button"
            (click)="shiftVisibleRange(-1)"
            [attr.aria-label]="isMobileLayout ? ('plans.gantt.range.prev_month' | translate) : null">
            @if (isMobileLayout) {
              <svg class="gantt-range-button__icon" viewBox="0 0 24 24" aria-hidden="true">
                <path
                  fill="currentColor"

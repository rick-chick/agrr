...
describe('PublicPlanPrivateValuePreviewComponent', () => {
  ...
  it('renders collapsed details by default with feature count in summary', () => {
    ...
    expect(details.querySelector('summary')?.textContent).toContain('ログイン後に使える機能');
    expect(details.querySelector('summary')?.textContent).toContain('3');
  });

  it('expands to show three features when summary is clicked', () => {
    fixture.detectChanges();

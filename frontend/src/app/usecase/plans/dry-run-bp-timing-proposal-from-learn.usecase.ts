...
export interface DryRunBpTimingProposalFromLearnInputDto {
  cropId: number;
...
@Injectable()
export class DryRunBpTimingProposalFromLearnUseCase {
  constructor(
    @Inject(CROP_SETUP_PROPOSAL_GATEWAY) private readonly gateway: CropSetupProposalGateway
  ) {}

  execute(dto: DryRunBpTimingProposalFromLearnInputDto): void {

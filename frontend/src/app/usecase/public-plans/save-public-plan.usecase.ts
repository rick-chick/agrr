...
export class SavePublicPlanUseCase implements SavePublicPlanInputPort {
  ...
  execute(dto: SavePublicPlanInputDto): void {
    this.publicPlanGateway.savePlan(dto.planId).subscribe({
      next: (response) => {
        if (response.success) {
          this.outputPort.present({ message: this.translate.instant('public_plans.plan_saved_successfully') });
        } else {
          this.outputPort.onError({ message: response.error || this.translate.instant('public_plans.failed_to_save_plan') });
        }
      },
      error: (err) => this.outputPort.onError({ message: err.message || this.translate.instant('public_plans.failed_to_save_plan') })
    });
  }
}
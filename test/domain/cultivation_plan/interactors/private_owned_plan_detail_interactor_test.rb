    ...
    module Interactors
      class PrivateOwnedPlanDetailInteractorTest < DomainLibTestCase
        ...
        test "dispatches success with mapped private plan detail" do
          ...
          @private_read_gateway.expects(:find_plan_read_snapshot_by_plan_id).with(plan_id: plan_id).returns(snapshot)
        ...
        test "dispatches not_found when plan owned by another user" do
          ...
          @private_read_gateway.expects(:find_plan_read_snapshot_by_plan_id).with(plan_id: plan_id).returns(

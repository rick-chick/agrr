...
class Domain::CultivationPlan::Dtos::TaskScheduleItemCompleteInputTest < DomainLibTestCase
  ...
  test "actual_date が空なら clock.today を使う" do
    dto = Domain::CultivationPlan::Dtos::TaskScheduleItemCompleteInput.from_completion_params(
      {},
      clock: @clock
    )
  ...
  test "不正な日付文字列は RecordInvalid" do
    err = assert_raises(Domain::Shared::Exceptions::RecordInvalid) do
      Domain::CultivationPlan::Dtos::TaskScheduleItemCompleteInput.from_completion_params(
        { "actual_date" => "bogus" },
        clock: @clock
      )
    end

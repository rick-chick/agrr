  ...
  module FieldCultivation
    module Interactors
      class FieldCultivationClimateDataInteractorTest < DomainLibTestCase
        ...
        test "merges observed weather across cultivation period when display range is narrower" do
          fc_id = 42
          source = build_source(
            fc_id: fc_id,
            start_date: Date.new(2026, 2, 17),
            completion_date: Date.new(2026, 7, 13),
          ...
          observed_period = nil
          progress_weather_payload = nil
          weather_data_gateway = Object.new
          weather_data_gateway.define_singleton_method(:weather_data_for_period) do |**kwargs|
            observed_period = kwargs
            [
              Struct.new(:date, :temperature_max, :temperature_min, :temperature_mean, :precipitation, :sunshine_hours, :wind_speed, :weather_code, keyword_init: true).new(
          ...
          progress_gateway = Object.new
          progress_gateway.define_singleton_method(:calculate_progress) do |**kwargs|
            progress_weather_payload = kwargs[:weather_payload]
            { "progress_records" => [] }
          end

          output_port = Minitest::Mock.new
          output_port.expect(:present, nil) { |_arg| true }
          ...
          interactor.instance_variable_set(:@output_port, output_port)
          interactor.instance_variable_set(:@clock, Struct.new(:today).new(Date.new(2026, 5, 31)))

          interactor.call(
            Domain::FieldCultivation::Dtos::FieldCultivationClimateDataInput.new(
              field_cultivation_id: fc_id,
              display_start_date: "2026-07-01",
              display_end_date: "2026-07-31"
            )
          )

          assert_equal Date.new(2026, 2, 17), observed_period[:start_date]
          assert_equal Date.new(2026, 5, 30), observed_period[:end_date]
          times = progress_weather_payload["data"].map { |datum| Date.parse(datum["time"]) }

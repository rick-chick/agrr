...
# HTML 応答形（redirect / 422 再表示）の境界のみ。一覧・詳細の絞り込みは CropList/DetailInteractor、
# ステージ・nutrients の永続化は test/integration または API masters が担保する。
class CropsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "edit は配線が通り nutrients フィールドを描画する" do
    sign_in_as @user

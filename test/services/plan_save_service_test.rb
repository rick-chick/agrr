...
class PlanSaveServiceTest < ActiveSupport::TestCase
  ...
  test "result success? reflects internal state" do
    result = PlanSaveSession::Result.new
  ...
  test "handles invalid farm_id by capturing RecordNotFound and returning failure result" do
    ...
    result = PlanSaveSession.new(user: @user, session_data: session_data).call
    assert_equal false, result.success
  ...
  test "copies crop task schedule blueprints from reference crops to user crops" do
    ...
    second_result = PlanSaveSession.new(user: @user, session_data: session_data).call
  ...
  test "should allow CultivationPlanCrop duplication when same crop appears multiple times" do
    ...
    # PlanSaveServiceを実行
    service = PlanSaveSession.new(user: @user, session_data: session_data)
    result = service.call
  ...
  test "reuses existing user farm when same reference farm is copied twice" do
    ...
    first_result = PlanSaveSession.new(user: @user, session_data: session_data).call
  ...
  test "raises error when reference agrr item loses gdd trigger" do
    ...
    error = assert_raises PlanSaveSession::InvalidTaskScheduleItemError do
      service.call
    end

require 'test_helper'
require Rails.root.join('db/migrate/20251107191500_data_migration_japan_reference_tasks')

class DataMigrationJapanReferenceTasksTest < ActiveSupport::TestCase
  EXPECTED_TASKS = DataMigrationJapanReferenceTasks::TASK_DEFINITIONS.transform_values do |definition|
  ...
  def setup
    ...
    reference_crop_names.each do |crop_name|
      Crop.where(name: crop_name, region: 'jp').delete_all
      create(:crop, :reference, name: crop_name, region: 'jp')
    end

    @migration = DataMigrationJapanReferenceTasks.new
  end
  ...
  def test_down_removes_reference_tasks_and_associations
    ...
    EXPECTED_TASKS.keys.each do |name|
      assert_nil AgriculturalTask.find_by(name: name, region: 'jp', is_reference: true)
    end

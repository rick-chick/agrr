...
| 領域 | 置き場 | ランナー |
|------|--------|----------|
| API/WS 契約（正） | `crates/agrr-r4-contract`（移行中）+ `test/contract/**` + co-located `agrr-server` | `run-rust-contract-tests.sh`（Rust ハーネス → Ruby 契約の順） |
| ドメイン | `crates/agrr-domain` | `run-test-rust-domain.sh` |
| 契約ハーネス（AR fixture） | `test/contract/**`, `test/factories/**` | `run-rust-contract-tests.sh` |
...
crates/agrr-r4-contract/   # P8.6.1: HTTP 契約（Ruby から段階移行。run-rust-contract-tests.sh が先に実行）
```

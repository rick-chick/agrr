...
module Domain
  module Backdoor
    module Interactors
      class BackdoorClearDatabaseInteractorTest < ActiveSupport::TestCase
        setup do
        ...
        test "success maps stats to success dto and logs summary" do
          ...
          @presenter.expects(:on_success).with do |dto|
            dto.is_a?(Dtos::BackdoorClearDatabaseSuccessDto) &&
        ...
        test "failure maps error message to failure dto" do
          ...
          @presenter.expects(:on_failure).with do |dto|
            dto.is_a?(Dtos::BackdoorClearDatabaseFailureDto) &&

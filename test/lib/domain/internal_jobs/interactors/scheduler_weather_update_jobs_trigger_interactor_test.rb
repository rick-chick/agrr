...
module Domain
  module InternalJobs
    module Interactors
      class SchedulerWeatherUpdateJobsTriggerInteractorTest < ActiveSupport::TestCase
        setup do
        ...
        test "failure maps message to failure dto" do
          ...
          @presenter.expects(:on_failure).with do |dto|
            dto.is_a?(Dtos::SchedulerWeatherUpdateTriggerFailureDto) && dto.message == "enqueue failed"
          end

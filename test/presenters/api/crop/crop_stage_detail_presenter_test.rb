...
require_relative '../../../../lib/presenters/api/crop/crop_stage_detail_presenter'

class CropStageDetailPresenterTest < ActiveSupport::TestCase
  test 'on_success calls view.render_response with ok status and serialized success data' do
    view_mock = mock
    presenter = Presenters::Api::Crop::CropStageDetailPresenter.new(view: view_mock)
  ...
  test 'on_failure calls view.render_response with not_found status and error message' do
    ...
    error_dto = Domain::Shared::Dtos::ErrorDto.new('Crop stage not found')

    expected_json = {
      error: 'Crop stage not found'
    }

...
class FertilizeDestroyHtmlPresenterTest < ActiveSupport::TestCase
  ...
  test 'on_success with undo event redirects with notice' do
    ...
    event.expects(:metadata).returns({ 'resource_label' => 'Test Fertilize' })

    destroy_output_dto = mock
    destroy_output_dto.expects(:undo).returns(event)

    view_mock.expects(:fertilizes_path).returns('/fertilizes')
    view_mock.expects(:redirect_back).with(fallback_location: '/fertilizes', notice: I18n.t('deletion_undo.redirect_notice', resource: 'Test Fertilize'))
  ...
  test 'on_success without undo token redirects with default notice' do
    ...
    view_mock.expects(:fertilizes_path).returns('/fertilizes')
    view_mock.expects(:redirect_to).with('/fertilizes', notice: I18n.t('fertilizes.flash.destroyed'))
  ...
  test 'on_failure redirects with alert' do
    ...
    view_mock.expects(:fertilizes_path).returns('/fertilizes')
    view_mock.expects(:redirect_back).with(fallback_location: '/fertilizes', alert: 'Test error')

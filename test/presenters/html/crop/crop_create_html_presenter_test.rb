...
class CropCreateHtmlPresenterTest < ActiveSupport::TestCase
  include Rails.application.routes.url_helpers
  test "on_success redirects with success notice" do
    view_mock = mock
    presenter = Presenters::Html::Crop::CropCreateHtmlPresenter.new(view: view_mock)

    crop_entity = mock
    crop_entity.expects(:id).returns(1)

    view_mock.expects(:crop_path).with(1).returns("/crops/1")
    view_mock.expects(:redirect_to).with("/crops/1", notice: I18n.t("crops.flash.created"))
  ...
  test "on_failure sets flash alert and renders new template" do
    ...
    view_mock.expects(:after_crop_create_failure)
  ...
  test "on_failure redirects to index when reference data requires admin" do
    ...
    view_mock.expects(:redirect_to).with("/crops", alert: msg)

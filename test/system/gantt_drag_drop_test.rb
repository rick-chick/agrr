...
class GanttDragDropTest < ApplicationSystemTestCase
  def setup
    # アノニマスユーザーを作成
    @user = User.create!(
  ...
  test "バーをドラッグすると視覚的フィードバックが表示される" do
    visit_results_page
    ...
    stroke_dasharray = page.evaluate_script(<<~JS)
      const bar = document.querySelector('.cultivation-bar .bar-bg');
      return bar ? bar.getAttribute('stroke-dasharray') : null;
    JS
    
    assert_equal "0.8", opacity, "ドラッグ中に透明度が変更されていません"
    assert_equal "4", stroke_width, "ドラッグ中に線幅が変更されていません"
    assert_equal "5,5", stroke_dasharray, "ドラッグ中に破線が表示されていません"
    
    take_screenshot
  end

  test "バーをドラッグして移動できる" do
    visit_results_page
    ...
    # バーの位置が変更されたか確認
    new_x = page.evaluate_script(<<~JS)
      const bar = document.querySelector('.cultivation-bar .bar-bg');
      return bar ? bar.getAttribute('x') : null;
    JS
    
    assert_not_equal original_x, new_x, "バーの位置が変更されていません"
  ...
  test "削除ボタンが表示される" do
    ...
    # 削除ボタンのスタイルを確認
    delete_btn_style = page.evaluate_script(<<~JS)
    ...
    assert_equal "pointer", delete_btn_style["cursor"], "削除ボタンにpointerカーソルが設定されていません"
    
    take_screenshot
  end

  test "削除ボタンをクリックすると確認ダイアログが表示される" do
    visit_results_page
  ...
  test "移動履歴が正しく記録される" do
    ...
    assert_not button_disabled, "移動後に再最適化ボタンが有効になっていません"
  ...
  def take_screenshot
    ...
    screenshot_path = Rails.root.join("tmp", "screenshots", "gantt_drag_drop_#{timestamp}.png")

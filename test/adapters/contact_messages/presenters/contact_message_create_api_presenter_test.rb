...
class ContactMessageCreateApiPresenterTest < ActiveSupport::TestCase
  test "on_success calls view.render_response with created status and serialized json" do
    view_mock = mock
    presenter = Adapters::ContactMessages::Presenters::ContactMessageCreateApiPresenter.new(view: view_mock)
  ...
  test "on_failure renders generic error with internal_server_error" do
    ...
    view_mock.expects(:render_response).with(
      json: { error: Adapters::ContactMessages::Presenters::ContactMessageCreateApiPresenter::INTERNAL_SERVER_ERROR_MESSAGE },

...
module Adapters
  module Crop
    class TaskScheduleBlueprintDeletionTest < ActiveSupport::TestCase
      test "deletes blueprint and template when no remaining blueprints" do
        ...
        service = TaskScheduleBlueprintDeletion.new(crop: crop, blueprint: blueprint)
        result = service.call
      ...
      test "propagates ActiveRecord::RecordNotFound when blueprint destroy raises" do
        blueprint = mock("blueprint")
        crop = mock("crop")

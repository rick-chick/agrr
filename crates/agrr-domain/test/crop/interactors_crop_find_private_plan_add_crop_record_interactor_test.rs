...
impl CropFindPrivatePlanAddCropRecordOutputPort for O {
    fn on_success(&mut self, _: CropEntity) {
        self.ok = true;
    }
...
fn crop(id: i64, user_id: Option<i64>, is_ref: bool) -> CropEntity {
    CropEntity {
        id,
        user_id,
        name: "C".into(),
...
struct UserGw;
impl UserLookupGateway for UserGw {
...
#[test]
fn accepts_user_owned_non_reference_crop() {
    let mut o = O {
        ok: false,
        fail: false,
    };
    let g = G {
        c: crop(53, Some(2), false),
        nf: false,
    };
    let u = UserGw;
    let mut i = CropFindPrivatePlanAddCropRecordInteractor::new(&mut o, 2, &g, &u, &Noop);
...
#[test]
fn rejects_other_users_crop() {
    let mut o = O {
        ok: false,
        fail: false,
    };
    let g = G {
        c: crop(99, Some(3), false),
        nf: false,
    };
    let u = UserGw;
    let mut i = CropFindPrivatePlanAddCropRecordInteractor::new(&mut o, 2, &g, &u, &Noop);

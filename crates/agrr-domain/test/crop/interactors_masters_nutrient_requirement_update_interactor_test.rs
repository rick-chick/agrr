// Tests for `interactors/masters_nutrient_requirement_update_interactor.rs` (Ruby parity under test/domain/crop/).
    ...
    impl CropGateway for CropGw {
        ...
        fn create_nutrient_requirement(&self, _: i64, _: NutrientRequirementUpdateInput) -> Result<NutrientRequirementEntity, Box<dyn std::error::Error + Send + Sync>> { unimplemented!() }
    ...
    fn updates_when_present() {
        ...
        let mut i = MastersNutrientRequirementUpdateInteractor::new(&mut out, &crop, &req);
        i.call(NutrientRequirementUpdateInput::new(
            1,
            2,
            json!({"daily_uptake_n": 3.0}),
        ))
        .unwrap();
    ...
    fn not_found_when_missing_before_update() {
        ...
        let mut out = Spy { event: None };
        let mut i = MastersNutrientRequirementUpdateInteractor::new(&mut out, &crop, &req);
        i.call(NutrientRequirementUpdateInput::new(1, 2, json!({})))
            .unwrap();

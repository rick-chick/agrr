// Tests for `interactors/crop_masters_task_template_index_interactor.rs` (Ruby parity under test/domain/crop/).

    use crate::crop::entities::CropEntity;
    use crate::shared::user::User;
    struct L(User); impl UserLookupGateway for L { fn find(&self, _: i64) -> User { self.0 } }
    struct O { rows: Option<Vec<serde_json::Value>> }
    impl CropMastersTaskTemplateIndexOutputPort for O {
        fn on_success(&mut self, rows: Vec<serde_json::Value>) { self.rows = Some(rows); }
    ...
    impl CropGateway for G {
        ...
        fn masters_crop_agricultural_task_templates_index_rows(&self, _: i64) -> Result<Vec<serde_json::Value>, Box<dyn std::error::Error + Send + Sync>> {
    ...
    // Ruby: test "should return rows successfully"
    #[test] fn rows_ok() {
        let mut o = O { rows: None };
        let logger_user_1 = L(User::new(1,false));
        let mut i = CropMastersTaskTemplateIndexInteractor::new(&mut o, &G, &logger_user_1);
        i.call(MastersCropTaskTemplateIndexInput { user_id: 1, crop_id: 1 }).unwrap();
        assert_eq!(o.rows.unwrap().len(), 1);
    }

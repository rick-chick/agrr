// Tests for `interactors/masters_temperature_requirement_create_interactor.rs` (Ruby parity under test/domain/crop/).
    ...
    impl TemperatureRequirementGateway for ReqGw {
        ...
        ) -> Result<Option<TemperatureRequirementEntity>, Box<dyn std::error::Error + Send + Sync>>
        {
            ...
            Ok(if self.exists {
                Some(TemperatureRequirementEntity::new(1, 2).unwrap())
            } else {
                None
            })
        }
    ...
    impl CropGateway for CropGw {
        fn create_temperature_requirement(
            ...
            Ok(TemperatureRequirementEntity::new(1, 2).unwrap())
        }
        ...
        fn update_temperature_requirement(&self, _: i64, _: TemperatureRequirementUpdateInput) -> Result<TemperatureRequirementEntity, Box<dyn std::error::Error + Send + Sync>> { unimplemented!() }
    ...
    fn sample_input() -> TemperatureRequirementUpdateInput {
        TemperatureRequirementUpdateInput::new(1, 2, json!({"base_temperature": 1.0}))
    }
    ...
    fn creates_when_absent_and_reports_success() {
        ...
        let mut i =
            MastersTemperatureRequirementCreateInteractor::new(&mut out, &crop, &req);
    ...
    fn reports_validation_errors_on_record_invalid() {
        ...
        let mut i =
            MastersTemperatureRequirementCreateInteractor::new(&mut out, &crop, &req);
        i.call(TemperatureRequirementUpdateInput::new(
            1,
            2,
            json!({"base_temperature": "bad"}),
        ))
        .unwrap();

// Tests for `interactors/crop_load_authorized_crop_stage_interactor.rs` (Ruby parity under test/domain/crop/).
    ...
    impl CropLoadedAuthorizationFailurePort for Nf {
        fn on_permission_denied(&mut self) {}
        fn on_not_found(&mut self) {
    ...
    // Ruby: test "returns bundle when crop and stage belong together"
    #[test]
    fn returns_bundle_when_crop_and_stage_belong_together() {
        let stage = CropStageEntity::new(2, 1, "s", 1).unwrap();
        let mut fp = NoFail;
        let cg = Cg(crop());
        let sg = Sg(stage.clone());
        let user_lookup = StubLookup(User::new(1, false));
        let mut i = CropLoadAuthorizedCropStageInteractor::new(
            &mut fp,
            9,
            &cg,
            &sg,
    ...
    // Ruby: test "returns nil when stage crop_id mismatches"
    #[test]
    fn returns_nil_when_stage_crop_id_mismatches() {
        let wrong = CropStageEntity::new(2, 99, "s", 1).unwrap();
        let mut fp = Nf {
        ...
        assert!(
            i.call(CropLoadAuthorizedCropStageInput::new(1, 2, false))
                .unwrap()
                .is_none()
        );
        assert!(fp.called.get());
    }

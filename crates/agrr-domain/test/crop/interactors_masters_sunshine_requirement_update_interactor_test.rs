// Tests for `interactors/masters_sunshine_requirement_update_interactor.rs` (Ruby parity under test/domain/crop/).
    ...
    impl CropGateway for CropGw {
        ...
        fn create_sunshine_requirement(&self, _: i64, _: SunshineRequirementUpdateInput) -> Result<SunshineRequirementEntity, Box<dyn std::error::Error + Send + Sync>> { unimplemented!() }
    ...
    fn updates_when_present() {
        ...
        let mut i = MastersSunshineRequirementUpdateInteractor::new(&mut out, &crop, &req);
        i.call(SunshineRequirementUpdateInput::new(
            1,
            2,
            json!({"minimum_sunshine_hours": 6.0}),
        ))
        .unwrap();
    ...
    fn not_found_when_missing_before_update() {
        ...
        let mut out = Spy { event: None };
        let mut i = MastersSunshineRequirementUpdateInteractor::new(&mut out, &crop, &req);
        i.call(SunshineRequirementUpdateInput::new(1, 2, json!({})))
            .unwrap();

// Tests for `interactors/masters_nutrient_requirement_create_interactor.rs` (Ruby parity under test/domain/crop/).
    ...
    impl NutrientRequirementGateway for ReqGw {
        ...
        ) -> Result<Option<NutrientRequirementEntity>, Box<dyn std::error::Error + Send + Sync>>
        {
            ...
            Ok(if self.exists {
                Some(NutrientRequirementEntity::new(1, 2).unwrap())
            } else {
                None
            })
        }
    ...
    impl CropGateway for CropGw {
        fn create_nutrient_requirement(
            ...
            Ok(NutrientRequirementEntity::new(1, 2).unwrap())
        }
        ...
        fn update_nutrient_requirement(&self, _: i64, _: NutrientRequirementUpdateInput) -> Result<NutrientRequirementEntity, Box<dyn std::error::Error + Send + Sync>> { unimplemented!() }
    ...
    fn sample_input() -> NutrientRequirementUpdateInput {
        NutrientRequirementUpdateInput::new(1, 2, json!({"daily_uptake_n": 1.0}))
    }
    ...
    fn creates_when_absent_and_reports_success() {
        ...
        let mut i =
            MastersNutrientRequirementCreateInteractor::new(&mut out, &crop, &req);
    ...
    fn reports_validation_errors_on_record_invalid() {
        ...
        let mut i =
            MastersNutrientRequirementCreateInteractor::new(&mut out, &crop, &req);
        i.call(NutrientRequirementUpdateInput::new(
            1,
            2,
            json!({"daily_uptake_n": "bad"}),
        ))
        .unwrap();

// Tests for `interactors/masters_temperature_requirement_update_interactor.rs` (Ruby parity under test/domain/crop/).
    ...
    impl CropGateway for CropGw {
        ...
        fn create_temperature_requirement(&self, _: i64, _: TemperatureRequirementUpdateInput) -> Result<TemperatureRequirementEntity, Box<dyn std::error::Error + Send + Sync>> { unimplemented!() }
    ...
    fn updates_when_present() {
        ...
        let mut i = MastersTemperatureRequirementUpdateInteractor::new(&mut out, &crop, &req);
        i.call(TemperatureRequirementUpdateInput::new(
            1,
            2,
            json!({"base_temperature": 3.0}),
        ))
        .unwrap();
    ...
    fn not_found_when_missing_before_update() {
        ...
        let mut out = Spy { event: None };
        let mut i = MastersTemperatureRequirementUpdateInteractor::new(&mut out, &crop, &req);
        i.call(TemperatureRequirementUpdateInput::new(1, 2, json!({})))
            .unwrap();

...
use crate::agricultural_task::constants::schedule_item_types::{
    ...
    #[test]
    fn generate_produces_pest_control_schedule_when_spray_blueprints_present() {
        ...
        let pest_control = replaced.iter().find(|r| r._category == "pest_control").unwrap();
        assert_eq!(pest_control.items.len(), 1);
        assert_eq!(pest_control.items[0].task_type, PREVENTIVE_SPRAY);
    }

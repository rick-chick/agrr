...
use crate::work_record::dtos::{WorkRecordClimateSnapshot, WorkRecordRead, WorkRecordTaskScheduleItemSummary};
...
struct StubClimateSnapshot {
    snapshot: WorkRecordClimateSnapshot,
}

impl WorkRecordClimateSnapshotGateway for StubClimateSnapshot {
    fn lookup(
        &self,
        _: i64,
        _: Date,
...
#[test]
fn create_persists_climate_snapshot_when_field_cultivation_present() {
    let events = Arc::new(Mutex::new(Vec::new()));
    let record_slot = Arc::new(Mutex::new(None));
    let mut output = SpyCreateOutput {
        events: Arc::clone(&events),
    ...
    let climate_snapshot = StubClimateSnapshot {
        snapshot: WorkRecordClimateSnapshot {
            gdd_at_actual: Some(142.5),
    ...
    let calls = create_calls.lock().unwrap();
    assert_eq!(Some(142.5), calls[0].1.gdd_at_actual);
    assert_eq!(

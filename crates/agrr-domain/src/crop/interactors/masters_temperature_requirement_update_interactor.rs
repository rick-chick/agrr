//! Ruby: `Domain::Crop::Interactors::MastersTemperatureRequirementUpdateInteractor`
...
pub struct MastersTemperatureRequirementUpdateInteractor<'a, G, RG, O> {
    output_port: &'a mut O,
    gateway: &'a G,
    requirement_gateway: &'a RG,
}

impl<'a, G, RG, O> MastersTemperatureRequirementUpdateInteractor<'a, G, RG, O>
where
    G: CropGateway,
    ...
    O: MastersTemperatureRequirementOutputPort,
{
    ...
    pub fn call(&mut self, input: TemperatureRequirementUpdateInput) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        match self.gateway.update_temperature_requirement(input.stage_id, input) {
            Ok(entity) => {
...
#[cfg(test)]
mod interactors_masters_temperature_requirement_update_interactor_test_inline {
    use super::*;
    include!(concat!(env!("CARGO_MANIFEST_DIR"), "/test/crop/interactors_masters_temperature_requirement_update_interactor_test.rs"));
}

//! Ruby: `Domain::Crop::Interactors::MastersSunshineRequirementUpdateInteractor`
...
pub struct MastersSunshineRequirementUpdateInteractor<'a, G, RG, O> {
    output_port: &'a mut O,
    gateway: &'a G,
    requirement_gateway: &'a RG,
}

impl<'a, G, RG, O> MastersSunshineRequirementUpdateInteractor<'a, G, RG, O>
where
    G: CropGateway,
    ...
    O: MastersSunshineRequirementOutputPort,
{
    ...
    pub fn call(
        ...
        match self.gateway.update_sunshine_requirement(input.stage_id, input) {
            Ok(entity) => {
...
#[cfg(test)]
mod interactors_masters_sunshine_requirement_update_interactor_test_inline {
    use super::*;
    include!(concat!(env!("CARGO_MANIFEST_DIR"), "/test/crop/interactors_masters_sunshine_requirement_update_interactor_test.rs"));
}

//! Ruby: `Domain::Crop::Interactors::MastersThermalRequirementDestroyInteractor`
...
pub struct MastersThermalRequirementDestroyInteractor<'a, G, O> {
    output_port: &'a mut O,
    gateway: &'a G,
}

impl<'a, G, O> MastersThermalRequirementDestroyInteractor<'a, G, O>
where
    G: CropGateway,
    O: MastersThermalRequirementOutputPort,
{
    ...
    pub fn call(&mut self, input: CropStageDetailInput) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        match self.gateway.delete_thermal_requirement(input.crop_stage_id) {
            Ok(()) => {
...
#[cfg(test)]
mod interactors_masters_thermal_requirement_destroy_interactor_test_inline {
    use super::*;
    include!(concat!(env!("CARGO_MANIFEST_DIR"), "/test/crop/interactors_masters_thermal_requirement_destroy_interactor_test.rs"));
}

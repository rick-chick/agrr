...
impl CropGateway for CropSqliteGateway {
    ...
    fn masters_crop_agricultural_task_templates_index_rows(
        ...
        self.pool.with_read_box(|conn| list_masters_crop_task_template_api_rows(conn, crop_id))
    }
    ...
    ) -> Result<UpdateMastersCropTaskTemplateOutcome, Box<dyn std::error::Error + Send + Sync>>
    {
        let m = attributes
            .as_object()
            .ok_or_else(|| Box::new(RecordInvalidError::new(Some("invalid attributes".into()), None))
                as Box<dyn std::error::Error + Send + Sync>)?;

        let mut errors = Vec::new();
        if let Some(name) = str_field(m, "name") {
            if name.trim().is_empty() {
                errors.push("Name can't be blank".into());
            }
        }
        if let Some(time_per_sqm) = f64_field(m, "time_per_sqm") {
            if time_per_sqm <= 0.0 {
                errors.push("Time per sqm must be greater than 0".into());
            }
        }
        if !errors.is_empty() {
            return Ok(UpdateMastersCropTaskTemplateOutcome::ValidationFailed { errors });
        }

        self.pool.with_write_box(|conn| {
            update_masters_crop_task_template(conn, crop_id, template_id, m)?;
            load_masters_crop_task_template_api_row(conn, crop_id, template_id)
        })
        .map(|row| UpdateMastersCropTaskTemplateOutcome::Ok { row })
    }
    ...
    fn delete_masters_crop_task_template(
        ...
        match self.pool.with_write_box(|conn| {
            conn.execute(
                "DELETE FROM crop_task_templates WHERE id = ?1 AND crop_id = ?2",
                params![template_id, crop_id],
            )
        }) {
            Ok(0) => Err(Box::new(RecordNotFoundError) as Box<dyn std::error::Error + Send + Sync>),
            Ok(_) => Ok(()),
...
pub(crate) fn required_tools_to_json(tools: &[String]) -> String {
    serde_json::to_string(tools).unwrap_or_else(|_| "[]".to_string())
}

/// Ruby: `masters_crop_task_template_api_row`
fn masters_crop_task_template_api_row_from_parts(
    template_id: i64,
    crop_id: i64,
    ...
    at_is_reference: Option<i64>,
) -> Value {
    let agricultural_task = at_id.map(|id| {
        json!({
            "id": id,
            "name": at_name.unwrap_or_default(),
            "description": at_description,
            "is_reference": at_is_reference.map(|v| v != 0).unwrap_or(false),
        })
    });
    json!({
        "id": template_id,
...
fn map_masters_crop_task_template_api_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<Value> {
    Ok(masters_crop_task_template_api_row_from_parts(
        row.get(0)?,
        row.get(1)?,
        row.get(2)?,
        row.get(3)?,
        ...
        row.get(5)?,
        row.get(6)?,
        parse_required_tools_json(row.get(7)?),
        row.get(8)?,
        row.get(9)?,
        row.get(10)?,
        row.get(11)?,
...
const MASTERS_CROP_TASK_TEMPLATE_SELECT: &str = "\
    SELECT ctt.id, ctt.crop_id, ctt.agricultural_task_id, ctt.name, ctt.description, \
           ctt.time_per_sqm, ctt.weather_dependency, ctt.required_tools, ctt.skill_level, \
           ctt.created_at, ctt.updated_at, \
           at.id, at.name, at.description, at.is_reference \
    FROM crop_task_templates ctt \
    LEFT JOIN agricultural_tasks at ON at.id = ctt.agricultural_task_id";

fn list_masters_crop_task_template_api_rows(
    conn: &rusqlite::Connection,
    crop_id: i64,
) -> rusqlite::Result<Vec<Value>> {
    let sql = format!(
        "{MASTERS_CROP_TASK_TEMPLATE_SELECT} WHERE ctt.crop_id = ?1 ORDER BY ctt.name ASC"
    );
    let mut stmt = conn.prepare(&sql)?;
    let rows = stmt.query_map(params![crop_id], map_masters_crop_task_template_api_row)?;
    let mut out = Vec::new();
    for row in rows {
        out.push(row?);
    }
...
fn load_masters_crop_task_template_api_row(
    conn: &rusqlite::Connection,
    crop_id: i64,
    template_id: i64,
) -> rusqlite::Result<Value> {
    let sql = format!(
        "{MASTERS_CROP_TASK_TEMPLATE_SELECT} WHERE ctt.id = ?1 AND ctt.crop_id = ?2 LIMIT 1"
    );
    conn.query_row(&sql, params![template_id, crop_id], map_masters_crop_task_template_api_row)
}

fn update_masters_crop_task_template(
    conn: &rusqlite::Connection,
    crop_id: i64,
    template_id: i64,
    m: &Map<String, Value>,
    ...
    if m.contains_key("description") {
        sets.push("description = ?".into());
        values.push(match m.get("description") {
            Some(Value::Null) | None => rusqlite::types::Value::Null,
            Some(v) => rusqlite::types::Value::Text(v.as_str().unwrap_or("").to_string()),
        });
    }
    if m.contains_key("time_per_sqm") {
        sets.push("time_per_sqm = ?".into());
        values.push(match f64_field(m, "time_per_sqm") {
            Some(v) => rusqlite::types::Value::Real(v),
            None => rusqlite::types::Value::Null,
        });
    }
    if let Some(weather_dependency) = str_field(m, "weather_dependency") {
        sets.push("weather_dependency = ?".into());
        values.push(rusqlite::types::Value::Text(weather_dependency));
    }
    if let Some(skill_level) = str_field(m, "skill_level") {
        sets.push("skill_level = ?".into());
        values.push(rusqlite::types::Value::Text(skill_level));
    }
    if let Some(tools) = m.get("required_tools").and_then(|v| v.as_array()) {
        let parsed: Vec<String> = tools
            .iter()
            .filter_map(|v| v.as_str().map(str::to_string))
            .collect();
        sets.push("required_tools = ?".into());
        values.push(rusqlite::types::Value::Text(required_tools_to_json(&parsed)));
    }

    if sets.is_empty() {
        return conn
            .query_row(
                "SELECT 1 FROM crop_task_templates WHERE id = ?1 AND crop_id = ?2",
                params![template_id, crop_id],
                |_| Ok(()),
            )
            .map(|_| ());
    ...
    sets.push("updated_at = datetime('now')".into());
    let sql = format!(
        "UPDATE crop_task_templates SET {} WHERE id = ? AND crop_id = ?",
        sets.join(", ")
    );
    values.push(rusqlite::types::Value::Integer(template_id));
    values.push(rusqlite::types::Value::Integer(crop_id));
...
#[cfg(test)]
mod masters_crop_task_template_tests {
    use super::*;
    use crate::pool::SqlitePool;
    ...
    fn temp_pool() -> SqlitePool {
        let dir = std::env::temp_dir().join(format!("agrr_crop_gw_{}", std::process::id()));
            ...
            conn.execute_batch(
                "CREATE TABLE crops (
                 ...
                 CREATE TABLE agricultural_tasks (
                   id INTEGER PRIMARY KEY, user_id INTEGER, name TEXT NOT NULL, description TEXT,
                   time_per_sqm REAL, weather_dependency TEXT, required_tools TEXT, skill_level TEXT,
                   region TEXT, task_type TEXT, is_reference INTEGER NOT NULL DEFAULT 0,
                   created_at TEXT, updated_at TEXT
                 );
                 CREATE TABLE crop_task_templates (
                   id INTEGER PRIMARY KEY, crop_id INTEGER NOT NULL, agricultural_task_id INTEGER,
                   name TEXT NOT NULL, description TEXT, time_per_sqm REAL, weather_dependency TEXT,
                   required_tools TEXT, skill_level TEXT, created_at TEXT, updated_at TEXT
                 );",
            )?;
            conn.execute(
                "INSERT INTO crops (id, user_id, name, is_reference, groups, created_at, updated_at)
                 VALUES (1, 10, 'Tomato', 0, '[]', '2026-01-01', '2026-01-01')",
                [],
            )?;
            conn.execute(
                "INSERT INTO agricultural_tasks (id, user_id, name, description, is_reference, required_tools, created_at, updated_at)
                 VALUES (20, 10, 'Watering', 'Water plants', 0, '[]', '2026-01-01', '2026-01-01')",
                [],
            )?;
            conn.execute(
                "INSERT INTO crop_task_templates (id, crop_id, agricultural_task_id, name, description, time_per_sqm, weather_dependency, required_tools, skill_level, created_at, updated_at)
                 VALUES (100, 1, 20, 'Water crop', 'Daily water', 0.5, 'low', '[\"hose\"]', 'beginner', '2026-01-01', '2026-01-01')",
                [],
            )?;
            Ok(())
        })
    ...
    #[test]
    fn masters_crop_agricultural_task_templates_index_rows_returns_api_rows() {
        let gw = CropSqliteGateway::new(temp_pool());
        let rows = gw
            .masters_crop_agricultural_task_templates_index_rows(1)
            .unwrap();
        assert_eq!(rows.len(), 1);
        assert_eq!(rows[0]["id"], 100);
        assert_eq!(rows[0]["name"], "Water crop");
        assert_eq!(rows[0]["agricultural_task"]["name"], "Watering");
        assert_eq!(rows[0]["required_tools"][0], "hose");
    }

    #[test]
    fn update_masters_crop_task_template_for_api_updates_and_returns_row() {
        let gw = CropSqliteGateway::new(temp_pool());
        let outcome = gw
            .update_masters_crop_task_template_for_api(
                1,
                100,
                json!({ "name": "Updated water", "time_per_sqm": 0.8 }),
            )
            .unwrap();
        match outcome {
            UpdateMastersCropTaskTemplateOutcome::Ok { row } => {
                assert_eq!(row["name"], "Updated water");
                assert_eq!(row["time_per_sqm"], 0.8);
            }
    ...
    #[test]
    fn update_masters_crop_task_template_for_api_validation_failed_on_blank_name() {
        let gw = CropSqliteGateway::new(temp_pool());
        let outcome = gw
            .update_masters_crop_task_template_for_api(1, 100, json!({ "name": "  " }))
            .unwrap();
        match outcome {
            UpdateMastersCropTaskTemplateOutcome::ValidationFailed { errors } => {
    ...
    #[test]
    fn update_masters_crop_task_template_for_api_not_found_for_wrong_crop() {
        let gw = CropSqliteGateway::new(temp_pool());
        let err = gw
            .update_masters_crop_task_template_for_api(99, 100, json!({ "name": "x" }))
            .unwrap_err();
        assert!(err.downcast_ref::<RecordNotFoundError>().is_some());
    }

    #[test]
    fn delete_masters_crop_task_template_removes_row() {
        let pool = temp_pool();
        let gw = CropSqliteGateway::new(pool.clone());
        gw.delete_masters_crop_task_template(1, 100).unwrap();
        let rows = gw
            .masters_crop_agricultural_task_templates_index_rows(1)
            .unwrap();
    ...
    #[test]
    fn delete_masters_crop_task_template_not_found() {
        let gw = CropSqliteGateway::new(temp_pool());
        let err = gw.delete_masters_crop_task_template(1, 999).unwrap_err();
        assert!(err.downcast_ref::<RecordNotFoundError>().is_some());
    }
}
